...
function readNearSigningSource() {
...
describeChecks('threshold Ed25519 near signing queue guard', () => {
  check('threshold near signing routes through strict session-scoped queue wrapper', () => {
  ...
  check('Email OTP NEAR signing waits for pending Ed25519 warm-up instead of falling through', () => {
  ...
  check('Email OTP NEAR transaction signing does not perform ECDSA sealed restore as a read side effect', () => {
    const nearSigning = readNearSigningSource();
  ...
  check('NEAR transaction step-up uses material-aware auth planning', () => {
    const transactionsFlow = readRepoSource(
      'packages/sdk-web/src/core/signingEngine/flows/signNear/signTransactions.ts',
    );
  ...
  check('Email OTP NEAR warm-session planning does not treat sealed records as spendable auth', () => {
    const nearSigning = readNearSigningSource();
  ...
  check('Email OTP NEAR signing records wallet-session budget spend without raw client-base cache', () => {
    const nearSigning = readNearSigningSource();
    const transactionsFlow = readRepoSource(
      'packages/sdk-web/src/core/signingEngine/flows/signNear/signTransactions.ts',
    );
  ...
  check('wallet-session reads do not treat pending Ed25519 records as logged-in capability', () => {
    const loginFlow = readRepoSource('packages/sdk-web/src/SeamsWeb/operations/auth/login.ts');
  ...
  check('normal Ed25519 signing flows do not run HSS material repair', () => {
    ...
    const materialRestore = readRepoSource(
      'packages/sdk-web/src/core/signingEngine/session/warmCapabilities/ed25519MaterialRestore.ts',
    );
  ...
  check('normal Ed25519 signing flows do not read raw client-base material', () => {
    const transactionFlow = readRepoSource(
      'packages/sdk-web/src/core/signingEngine/flows/signNear/signTransactions.ts',
    );
    const nep413Flow = readRepoSource(
  ...
  check('active NEAR Ed25519 signing planners use classified worker-material state', () => {
    const scannedPaths = [
      'packages/sdk-web/src/core/signingEngine/flows/signNear/signNear.ts',
      'packages/sdk-web/src/core/signingEngine/flows/signNear/signTransactions.ts',
      'packages/sdk-web/src/core/signingEngine/flows/signNear/signNep413.ts',
  ...
  check('Email OTP NEAR cached client-base signing consumes shared session budgets without PRF claim', () => {
  ...
  check('transaction signing does not consume worker warm-session budgets directly', () => {
    const transactionsFlow = readRepoSource(
      'packages/sdk-web/src/core/signingEngine/flows/signNear/signTransactions.ts',
    );
  ...
  check('export code cannot consume signing grant budget', () => {
  ...
  check('user-facing Ed25519 signing flows stay on Router A/B normal signing', () => {
    const transactionFlow = readRepoSource(
      'packages/sdk-web/src/core/signingEngine/flows/signNear/signTransactions.ts',
    );
    const nep413Flow = readRepoSource(
...
console.log('[check-threshold-ed25519-near-signing-queue] passed');
