...
test.describe = function describe(_name, fn) {
  fn();
};
  ...
  return {
    ...
    not: {
      ...
      toHaveProperty(expected) {
        assert.ok(
          actual == null || !Object.prototype.hasOwnProperty.call(actual, expected),
          message ?? `expected object not to have own property ${String(expected)}`,
        );
      },
    },
  };
...
function listRepoSourceFiles(relativePath) {
...
function extractLegacyRouterAbRouteLiterals(source) {
    const routes = [];
    const literalPattern = /(['"`])([^'"`]*(?:\/v1\/hss|\/router-ab\/v1|\/router-ab\/do\/v1|\/session\/signing-budget)[^'"`]*)\1/g;
    let match;
    while ((match = literalPattern.exec(source))) {
        ...
        const pathStart = [hssIndex, privateIndex, durableObjectIndex, walletBudgetIndex]
...
function constArraySource(source, constName) {
...
function singleQuotedFieldValues(source, field) {
    return [...source.matchAll(new RegExp(`\\b${field}:\\s*'([^']+)'`, 'g'))].map((match) => match[1] || '');
}
...
function assertSourceOwnership(expectations) {
    const offenders = [];
...
test.describe('Router A/B normal-signing SDK source guards', () => {
    ...
    test('pnpm router launches one public Router server and three private workers', () => {
        const packageJson = JSON.parse(readRepoSource('package.json'));
        expect(packageJson.scripts?.router).toBe('node ./crates/router-ab-dev/scripts/dev-local-workers.mjs --mode logs');
        expect(packageJson.scripts?.['router:multiplex']).toBe('node ./crates/router-ab-dev/scripts/dev-local-workers.mjs --mode multiplex');
        ...
        expect(packageJson.scripts?.site).toContain('VITE_ROUTER_AB_NORMAL_SIGNING_WORKER_ID=local-signing-worker');
    ...
    test('SDK helper surface uses Wallet Session v2 request builders only', () => {
        const relayerHttpSource = readRepoSource('packages/sdk-web/src/core/rpcClients/relayer/relayerHttp.ts');
        const sources = [
            ...
            'packages/sdk-web/src/core/signingEngine/session/warmCapabilities/routerAbWalletSessionCredential.ts',
        ].map((relativePath) => ({
            relativePath,
            source: readRepoSource(relativePath),
        }));
        ...
        const offenders = sources.flatMap(({ relativePath, source }) => forbiddenMarkers
        ...
        expect(sources[1].source).toContain('secure randomness is unavailable for Router A/B client presign id');
        expect(sourceRangeBetween(sources[1].source, 'const prepareResponse = await prepareRouterAbNormalSigningV2({', 'const clientShare =')).toContain('requireRouterAbNormalSigningPrepareMatchesRequest({');
    ...
    test('ECDSA active signing reads Wallet Session auth from Router A/B ready state only', () => {
        ...
        expect(offenders, offenders.join('\n')).toEqual([]);
        const activeSigningSource = readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signEvmFamily/signEvmFamily.ts');
        const ecdsaAuthPlanningSource = readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signEvmFamily/authPlanning.ts');
        ...
        expect(ecdsaAuthPlanningSource).not.toContain('classifyThresholdEcdsaSessionRecordRoleLocalState');
    });
    test('Router A/B signing readiness has no implicit persistence fallback paths', () => {
        ...
        const statusReaderSource = readRepoSource('packages/sdk-web/src/core/signingEngine/session/warmCapabilities/statusReader.ts');
        for (const marker of [
        ...
        expect(readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signEvmFamily/signingFlow.ts')).toContain('resolveEcdsaSigningMaterialPlan');
    });
    test('Ed25519 active signing reads Wallet Session auth from Router A/B ready state only', () => {
        ...
        expect(offenders, offenders.join('\n')).toEqual([]);
        const routerAbExecutorSource = readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signNear/shared/ed25519PresignFinalize.ts');
        const materialReadinessSource = readRepoSource('packages/sdk-web/src/core/signingEngine/session/warmCapabilities/ed25519SigningMaterialReadiness.ts');
        ...
        const walletSessionCredentialSource = readRepoSource('packages/sdk-web/src/core/signingEngine/session/warmCapabilities/routerAbWalletSessionCredential.ts');
        expect(walletSessionCredentialSource).toContain("requireNonEmpty(signingWalletSession.auth.walletSessionJwt, 'Wallet Session bearer JWT')");
    ...
    test('Ed25519 passkey reconnect prepare binds Router A/B normal-signing state', () => {
        const source = readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signNear/signNear.ts');
        ...
        expect(prepareSource).toContain('routerAbNormalSigning: thresholdSessionRecord.routerAbNormalSigning');
    });
    test('Ed25519 Router A/B final signing consumes worker material handles', () => {
        ...
        activeSigningSources.push({
            relativePath: 'packages/sdk-web/src/core/signingEngine/flows/signNear/shared/ed25519PresignFinalize.ts',
            source: readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signNear/shared/ed25519PresignFinalize.ts').slice(readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signNear/shared/ed25519PresignFinalize.ts').indexOf('async function tryFinalizeRouterAbEd25519NormalSigningSignature')),
        });
        ...
        expect(offenders, offenders.join('\n')).toEqual([]);
        const activeFinalSigningSources = activeSigningSources.filter(({ relativePath }) => !relativePath.includes('/shared/'));
        const finalSigningOffenders = [];
        for (const { relativePath, source } of activeFinalSigningSources) {
            if (!source.includes('requireOrRestoreRouterAbEd25519WalletSessionState')) {
                finalSigningOffenders.push(`${relativePath} does not resolve worker-owned Ed25519 material state`);
            }
            ...
            if (!source.includes('if (isThresholdSignerRepairableMaterialError(err))')) {
                finalSigningOffenders.push(`${relativePath} is missing repairable-material fail-closed branch`);
            }
            ...
            if (source.includes('ensureThresholdEd25519HssSigningMaterial(')) {
                finalSigningOffenders.push(`${relativePath} uses reconstruction-capable material loading before repair`);
            }
        }
        ...
        expect(readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signNear/shared/ed25519PresignFinalize.ts')).not.toContain('createThresholdEd25519RoleSeparatedNormalSigningClientShareFromMaterialHandleWasm');
        const ed25519PresignFinalizeSource = readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signNear/shared/ed25519PresignFinalize.ts');
        expect(ed25519PresignFinalizeSource).not.toContain('xClientBaseB64u');
        expect(ed25519PresignFinalizeSource).not.toContain('@/core/signingEngine/threshold/crypto/hssClientSignerWasm');
        expect(ed25519PresignFinalizeSource).toContain('createThresholdEd25519ClientPresignFromMaterialHandleWasm');
        expect(ed25519PresignFinalizeSource).toContain('signThresholdEd25519ClientPresignFromMaterialHandleWasm');
    });
    test('Ed25519 availability and auth planning consume classified material state', () => {
        const availabilitySource = readRepoSource('packages/sdk-web/src/core/signingEngine/session/availability/persistedAvailableSigningLanes.ts');
        const authPlanSource = readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signNear/shared/ed25519MaterialAuthPlan.ts');
        const preConfirmReadinessSource = readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signNear/shared/ed25519PreConfirmMaterialReadiness.ts');
        const offenders = [];
        if (!availabilitySource.includes('classifyRouterAbEd25519PersistedSigningRecord(args.record)')) {
    ...
    test('ECDSA-HSS Router A/B presign and signing consume worker material handles', () => {
        const poolSource = readRepoSource('packages/sdk-web/src/core/signingEngine/routerAb/ecdsaHss/presignaturePool.ts');
        const secp256k1Source = readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signEvmFamily/signers/secp256k1.ts');
        const loginPrefillSource = readRepoSource('packages/sdk-web/src/core/signingEngine/session/warmCapabilities/ecdsaLoginPrefill.ts');
        const loginPrefillMaterialSource = readRepoSource('packages/sdk-web/src/core/signingEngine/session/warmCapabilities/ecdsaLoginPrefillSigningMaterialSource.ts');
        const materialSource = readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signEvmFamily/signers/ecdsaHssClientSigningMaterialSource.ts');
        ...
        const publicSigningInputMatches = [
            ...poolSource.matchAll(/export async function signRouterAbEcdsaHssDigestWithPool(?:Hit)?\(args: \{[\s\S]*?\n\}/g),
        ].map((match) => match[0]);
        expect(publicSigningInputMatches).toHaveLength(2);
        expect(publicSigningInputMatches.join('\n')).toContain('clientSigningMaterial: RouterAbEcdsaHssClientSigningMaterialSource');
        for (const publicInput of publicSigningInputMatches) {
            expect(publicInput).not.toContain('clientSigningShare32:');
        }
        expect(materialSource).toContain("kind: 'router_ab_ecdsa_hss_client_signing_material_source_v1'");
        ...
        expect(materialSource).toContain('thresholdEcdsaRoleLocalComputeSignatureShareFromPresignatureHandleWasm');
        expect(materialSource).toContain('ensureRoleLocalSigningMaterialLoaded');
        expect(materialSource).toContain('storeEcdsaRoleLocalSigningMaterialWasm');
        expect(materialSource).not.toContain('roleLocalReadyRecordForWorkerRestore');
        expect(materialSource).toContain('stateBlob: material.stateBlob');
        ...
        expect(loginPrefillMaterialSource).toContain('storeEcdsaRoleLocalSigningMaterialWasm');
        ...
        expect(loginPrefillMaterialSource).toContain('initRouterAbEcdsaHssClientPresignSessionFromAdditiveShare');
        expect(loginPrefillMaterialSource).toContain('thresholdEcdsaRoleLocalPresignSessionInitFromMaterialHandleWasm');
        expect(loginPrefillMaterialSource).toContain('thresholdEcdsaRoleLocalComputeSignatureShareFromPresignatureHandleWasm');
        const warmSigningSource = readRepoSource('packages/sdk-web/src/core/signingEngine/assembly/ports/warmSigning.ts');
    ...
    test('ECDSA-HSS raw client signing share references stay in named temporary boundaries', () => {
        ...
        const offenders = sourcePaths.flatMap((relativePath) => {
            if (allowedRawSharePaths.has(relativePath))
                return [];
            const source = readRepoSource(relativePath);
    ...
    test('browser registration clients use shared Router A/B claim boundaries', () => {
        const source = readRepoSource('packages/sdk-web/src/core/rpcClients/relayer/walletRegistration.ts');
        const forbiddenMarkers = [
    ...
    test('SDK Wallet Session route auth uses current bearer-only discriminators', () => {
        ...
        const ecdsaBearerOnlyBoundaryPaths = activeAuthBoundaryPaths.filter((relativePath) => relativePath !== 'packages/shared-ts/src/utils/sessionTokens.ts');
        ...
        expect(offenders, offenders.join('\n')).toEqual([]);
        expect(readRepoSource('packages/shared-ts/src/utils/sessionTokens.ts')).toContain("kind: 'wallet_session'");
    });
    test('signing-capable Wallet Session issuance stays JWT-only inside SDK internals', () => {
        ...
        expect(offenders, offenders.join('\n')).toEqual([]);
        expect(readRepoSource('packages/sdk-web/src/core/signingEngine/session/routerAbSigningWalletSession.ts')).toContain("record.thresholdSessionKind !== 'jwt'");
    });
    ...
    test('Ed25519 registration and passkey reconnect avoid legacy client-base prewarm', () => {
        const offenders = [];
        const warmBootstrap = readRepoSource('packages/sdk-web/src/SeamsWeb/operations/session/thresholdWarmSessionBootstrap.ts');
        const registration = readRepoSource('packages/sdk-web/src/SeamsWeb/operations/registration/registration.ts');
        ...
        const signNear = readRepoSource('packages/sdk-web/src/core/signingEngine/flows/signNear/signNear.ts');
        ...
        if (!signNear.includes("case 'material_hint_unvalidated'") ||
    ...
    test('legacy public threshold signing surfaces stay confined to known deletion blockers', () => {
        ...
        for (const relativePath of scannedPaths) {
            ...
            if (source.includes('local_threshold_ecdsa_presignature_pool') &&
                !localEcdsaPoolFillAllowedFiles.includes(relativePath)) {
                offenders.push(`${relativePath} contains local_threshold_ecdsa_presignature_pool outside persisted ECDSA presign record cleanup surfaces`);
            }
        }
        const thresholdEntrypoint = readRepoSource('packages/sdk-web/src/threshold.ts');
        for (const token of ['connectEd25519Session', 'connectEcdsaSession']) {
    ...
    test('Router A/B legacy route literals are rejected from active source', () => {
        ...
        const offenders = scannedPaths.flatMap((relativePath) => {
            const source = readRepoSource(relativePath);
            return extractLegacyRouterAbRouteLiterals(source).map((route) => `${relativePath} contains legacy Router A/B route ${route}`);
        });
        expect(offenders, offenders.join('\n')).toEqual([]);
    });
    test('public normal-signing route cores keep server budget reservation hooks', () => {
        ...
        const ed25519Core = sourceRangeBetween(source, 'export async function handleRouterAbEd25519NormalSigningRouteCore', 'export function validateRouterAbEd25519NormalSigningRequestScope');
        const ecdsaHssCore = sourceRangeBetween(source, 'export async function handleRouterAbEcdsaHssNormalSigningRouteCore', 'export async function postRouterAbSigningWorkerJson');
        const forwardThenCommitHelper = sourceRangeBetween(source, 'async function forwardThenCommitBudgetedSigning', 'function stripRouterAbBudgetMetadata');
    ...
    test('SDK budget projection uses committed remainingUses as active signing authority', () => {
        const source = readRepoSource('packages/sdk-web/src/core/signingEngine/session/budget/budgetProjection.ts');
        const activeStatusType = sourceRangeBetween(source, 'export type TrustedWalletBudgetStatus =', 'export type WalletBudgetUnknownReason =');
    ...
    test('wallet budget success reconciliation never falls back to local warm-session consume ports', () => {
        const source = readRepoSource('packages/sdk-web/src/core/signingEngine/session/SigningSessionCoordinator.ts');
        const constructorSource = sourceRangeBetween(source, 'constructor(deps: SigningSessionCoordinatorDeps = {})', 'resolveAuthPlan(');
        const syncSource = sourceRangeBetween(source, 'private syncServerConsumedSpendStatus = async', 'async clear(');
        expect(constructorSource).toContain('syncSuccessfulSpendStatus: deps.consumeUse || this.syncServerConsumedSpendStatus');
...
console.log('[router-ab-normal-signing-sdk-boundaries] ok');
