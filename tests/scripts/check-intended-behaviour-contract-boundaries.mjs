...
const mutationSelfCheckManifestPath = path.join(repoRoot, intendedRoot, 'mutation-self-check.manifest.json');
...
const intendedMutationSelfCheckScriptPath = path.join(repoRoot, 'tests/scripts/check-intended-mutation-self-check.mjs');
const intendedGoogleOidcEnvModulePath = path.join(repoRoot, 'tests/scripts/intended-google-oidc-env.mjs');
const intendedGoogleOidcSetupScriptPath = path.join(repoRoot, 'tests/scripts/setup-intended-google-oidc.mjs');
const intendedGoogleTokenRefreshScriptPath = path.join(repoRoot, 'tests/scripts/refresh-intended-google-token.mjs');
const intendedGoogleTokenEnsureScriptPath = path.join(repoRoot, 'tests/scripts/ensure-intended-google-token.mjs');
const intendedLocalConsoleSeedScriptPath = path.join(repoRoot, 'tests/scripts/seed-intended-local-console.mjs');
const warmSessionTestServicesFixturePath = path.join(repoRoot, 'tests/unit/helpers/warmSessionTestServices.fixtures.ts');
const signingSessionRecordFixturePath = path.join(repoRoot, 'tests/unit/helpers/signingSessionRecord.fixtures.ts');
const warmSessionUiConfirmFixturePath = path.join(repoRoot, 'tests/unit/helpers/warmSessionUiConfirm.fixtures.ts');
const ecdsaChainTargetFixturePath = path.join(repoRoot, 'tests/unit/helpers/ecdsaChainTarget.fixtures.ts');
const ecdsaBootstrapFixturePath = path.join(repoRoot, 'tests/unit/helpers/ecdsaBootstrap.fixtures.ts');
...
const walletServiceHeadersTestPath = path.join(repoRoot, 'tests/e2e/wallet-service-headers.test.ts');
...
const registrationFlowBenchmarkReportPath = path.join(repoRoot, 'docs/benchmarks/registration-flow.md');
...
const expectedMutationProofStatuses = {
    cross_chain_ecdsa_material_reuse: 'detected',
...
const expectedMutationDetectedProofEvidence = {
    cross_chain_ecdsa_material_reuse: {
        ...
        observedFailureCommand: 'pnpm -C tests exec playwright test -c playwright.intended.ci.config.ts e2e/intended-behaviours/passkey.registration.contract.test.ts e2e/intended-behaviours/passkey.unlock.contract.test.ts --reporter=line',
        restoredValidationCommand: 'pnpm -C tests exec playwright test -c playwright.intended.ci.config.ts e2e/intended-behaviours/passkey.registration.contract.test.ts e2e/intended-behaviours/passkey.unlock.contract.test.ts --reporter=line',
    },
    email_otp_reroll_bootstrap_token_request_mismatch: {
        observedAt: '2026-07-04',
        observedFailureCommand: 'pnpm -C tests run ensure:intended-google-token && SEAMS_INTENDED_SIGNING_SESSION_DEBUG=1 pnpm -C tests exec playwright test -c playwright.intended.config.ts e2e/intended-behaviours/email-otp.registration.contract.test.ts --reporter=line',
        restoredValidationCommand: 'pnpm -C tests run ensure:intended-google-token && SEAMS_INTENDED_SIGNING_SESSION_DEBUG=1 pnpm -C tests exec playwright test -c playwright.intended.config.ts e2e/intended-behaviours/email-otp.registration.contract.test.ts --reporter=line',
    },
    export_provider_user_mismatch_after_app_session_refresh: {
        observedAt: '2026-07-04',
        observedFailureCommand: 'pnpm -C tests run ensure:intended-google-token && SEAMS_INTENDED_SIGNING_SESSION_DEBUG=1 pnpm -C tests exec playwright test -c playwright.intended.config.ts e2e/intended-behaviours/email-otp.registration.contract.test.ts --reporter=line',
    ...
    first_post_step_up_transaction_failure: {
        observedAt: '2026-07-04',
        observedFailureCommand: 'pnpm -C tests exec playwright test -c playwright.intended.config.ts e2e/intended-behaviours/passkey.registration.contract.test.ts e2e/intended-behaviours/passkey.unlock.contract.test.ts --reporter=line',
        restoredValidationCommand: 'pnpm -C tests exec playwright test -c playwright.intended.config.ts e2e/intended-behaviours/passkey.registration.contract.test.ts e2e/intended-behaviours/passkey.unlock.contract.test.ts --reporter=line',
    },
};
const expectedIntendedActionResultKinds = [
    ...
    'email_recovery_finalize_success',
    'email_recovery_start_success',
    'near_sign_success',
...
function isIntendedBehaviourBoundaryChecker(relativePath) {
    return (relativePath === 'tests/unit/intendedBehaviourContracts.guard.unit.test.ts' ||
...
test('Refactor 88 intended harness owns setup primitives outside legacy setup', () => {
    ...
    for (const specifier of imports) {
        if (!specifier.includes('tests/setup') && !specifier.includes('../setup'))
            continue;
...
test('Refactor 88 intended action result discriminants stay aligned', () => {
    const pageSource = fs.readFileSync(path.join(repoRoot, 'apps/seams-site/src/pages/intended-e2e/page.tsx'), 'utf8');
    const harnessSource = fs.readFileSync(intendedHarnessPath, 'utf8');
    ...
    expect(extractUniqueSwitchCasesBeforeDefault(extractDelimitedSource(harnessSource, 'function parseIntendedActionResultSnapshot', 'function parseEcdsaEnabledSnapshot'), 'switch (kind)')).toEqual([...expectedIntendedActionResultKinds]);
    expect(extractUniqueSwitchCasesBeforeDefault(extractDelimitedSource(pageSource, 'function intendedActionResultWalletId', 'function intendedActionResultNearAccountId'), 'switch (result.kind)')).toEqual([...expectedIntendedActionResultKinds]);
    expect(extractUniqueSwitchCasesBeforeDefault(extractDelimitedSource(pageSource, 'function intendedActionResultNearAccountId', 'function readIntendedPageQuery'), 'switch (result.kind)')).toEqual([...expectedIntendedActionResultKinds]);
});
test('Refactor 88 retired mocked runtime files stay deleted', () => {
    const presentFiles = retiredMockedRuntimeFiles.filter((relativePath) => fs.existsSync(path.join(repoRoot, relativePath)));
    expect(presentFiles, presentFiles.join('\n')).toEqual([]);
});
test('Refactor 88 fake relay server launcher files stay deleted', () => {
    const presentFiles = retiredFakeRelayServerFiles.filter((relativePath) => fs.existsSync(path.join(repoRoot, relativePath)));
    expect(presentFiles, presentFiles.join('\n')).toEqual([]);
});
...
test('Refactor 88 generic setup bootstrap stays out of lifecycle e2e tests', () => {
    ...
    for (const relativePath of allowedE2eLegacySetupBootstrapFiles) {
        if (retainedBoundaryAuditRowSet.has(relativePath))
            continue;
    ...
    for (const relativePath of listSourceTextFiles(['tests/e2e'])) {
        const source = fs.readFileSync(path.join(repoRoot, relativePath), 'utf8');
        if (!source.includes('setupBasicPasskeyTest'))
            continue;
...
test('Refactor 88 remaining generic browser bootstrap consumers stay audited', () => {
    ...
    for (const relativePath of listTypeScriptFiles(path.join(repoRoot, 'tests'))) {
        if (relativePath === 'tests/setup/index.ts')
            continue;
        if (isIntendedBehaviourBoundaryChecker(relativePath))
            continue;
        const source = fs.readFileSync(path.join(repoRoot, relativePath), 'utf8');
...
test('Refactor 88 retained boundary tests stay explicitly classified', () => {
    ...
    const walletIframeAuditRows = retainedBoundaryAuditRows.filter((relativePath) => relativePath.startsWith('tests/wallet-iframe/'));
    const walletIframeTestFiles = listTypeScriptFiles(path.join(repoRoot, 'tests/wallet-iframe')).filter((relativePath) => relativePath.endsWith('.test.ts'));
    ...
    for (const relativePath of retainedBoundaryAuditRows) {
        ...
        const missingEvidenceTokens = missingSourceTokens(source, retainedBoundaryAuditEvidenceTokens[relativePath]);
        ...
        if (planSource.includes(rowPrefix))
            continue;
...
test('Refactor 88 retained Lit component browser tests stay explicitly classified', () => {
    const litComponentAuditRows = retainedBoundaryAuditRows.filter((relativePath) => relativePath.startsWith('tests/lit-components/'));
    const litComponentTestFiles = listTypeScriptFiles(path.join(repoRoot, 'tests/lit-components')).filter((relativePath) => relativePath.endsWith('.test.ts'));
...
test('Refactor 88 SeamsWeb browser setup unit tests stay explicitly classified', () => {
    ...
    for (const relativePath of listTypeScriptFiles(path.join(repoRoot, 'tests/unit'))) {
        if (!path.basename(relativePath).startsWith('seamsWeb.'))
            continue;
        const source = fs.readFileSync(path.join(repoRoot, relativePath), 'utf8');
        if (!source.includes('setupBasicPasskeyTest'))
            continue;
...
test('Refactor 88 confirm-flow browser unit tests stay explicitly classified', () => {
    const confirmFlowAuditRows = retainedBoundaryAuditRows.filter((relativePath) => path.basename(relativePath).startsWith('confirmTxFlow.'));
...
test('Refactor 88 test README advertises intended contracts instead of retired lifecycle tests', () => {
    ...
    expect(source).toContain('`test`, `test:lite`, and `test:inline` use the Vite-only browser setup');
    expect(source).toContain('`test:e2e` uses the same generic config and excludes intended contracts');
...
test('Refactor 88 cleanup docs advertise Vite-only integration signing scripts', () => {
    ...
    expect(source).toContain('"test:integration:signing": "pnpm -C .. build:sdk-full && playwright test -c playwright.integration.config.ts --reporter=line"');
    expect(source).not.toContain('USE_RELAY_SERVER');
});
...
test('Refactor 88 browser test utilities do not expose retired mock hooks', () => {
    ...
    for (const relativePath of setupSurfaceFiles) {
        ...
        for (const retiredField of retiredBrowserTestUtils) {
            if (!source.includes(retiredField))
                continue;
...
test('Refactor 88 generic setup does not expose same-origin rewrite flags', () => {
    ...
    const setupBootstrapSource = fs.readFileSync(path.join(repoRoot, 'tests/setup/bootstrap.ts'), 'utf8');
...
test('Refactor 88 ECDSA chain target fixtures stay split from warm-session runtime fixtures', () => {
    const warmSessionTestServicesFixtureSource = fs.readFileSync(warmSessionTestServicesFixturePath, 'utf8');
    const signingSessionRecordFixtureSource = fs.readFileSync(signingSessionRecordFixturePath, 'utf8');
    const warmSessionUiConfirmFixtureSource = fs.readFileSync(warmSessionUiConfirmFixturePath, 'utf8');
    ...
    expect(warmSessionTestServicesFixtureSource).not.toMatch(/export function testEcdsaChain(Target|Id)\b/);
    expect(warmSessionTestServicesFixtureSource).not.toContain('export function createThresholdEcdsaBootstrapFixture');
    expect(warmSessionTestServicesFixtureSource).not.toContain('export function createThresholdEcdsaStoreFixture');
    expect(warmSessionTestServicesFixtureSource).not.toContain('export function seedEd25519WarmSessionRecord');
    expect(warmSessionTestServicesFixtureSource).not.toContain('export function seedEcdsaWarmSessionRecord');
    expect(warmSessionTestServicesFixtureSource).not.toContain('export function createWarmSessionStatusReader');
    expect(warmSessionTestServicesFixtureSource).not.toContain('export function createWarmSessionUiConfirmFixture');
    expect(warmSessionTestServicesFixtureSource).toContain('export function createWarmSessionTestServices');
    expect(signingSessionRecordFixtureSource).toContain('export function createThresholdEcdsaStoreFixture');
    expect(signingSessionRecordFixtureSource).toContain('export function seedEd25519WarmSessionRecord');
    expect(signingSessionRecordFixtureSource).toContain('export function seedEcdsaWarmSessionRecord');
    expect(warmSessionUiConfirmFixtureSource).toContain('export function createWarmSessionStatusReader');
    expect(warmSessionUiConfirmFixtureSource).toContain('export function createWarmSessionUiConfirmFixture');
...
test('Refactor 88 intended command is wired as a named pre-merge gate', () => {
    ...
    expect(readScript(testsPackage, 'test:intended')).toBe('pnpm run ensure:intended-google-token && playwright test -c playwright.intended.config.ts --reporter=line');
    expect(readScript(rootPackage, 'test:intended:ci')).toBe('pnpm -C tests test:intended:ci');
    expect(readScript(testsPackage, 'test:intended:ci')).toBe('pnpm run ensure:intended-google-token && playwright test -c playwright.intended.ci.config.ts --reporter=line');
    expect(readScript(rootPackage, 'ensure:intended-google-token')).toBe('pnpm -C tests ensure:intended-google-token');
    expect(readScript(testsPackage, 'ensure:intended-google-token')).toBe('node scripts/ensure-intended-google-token.mjs');
    expect(readScript(rootPackage, 'setup:intended-google-oidc')).toBe('pnpm -C tests setup:intended-google-oidc');
    expect(readScript(testsPackage, 'setup:intended-google-oidc')).toBe('node scripts/setup-intended-google-oidc.mjs');
    expect(readScript(rootPackage, 'refresh:intended-google-token')).toBe('pnpm -C tests refresh:intended-google-token');
    expect(readScript(testsPackage, 'refresh:intended-google-token')).toBe('node scripts/refresh-intended-google-token.mjs');
    expect(readScript(rootPackage, 'seed:intended-local-console')).toBe('pnpm -C tests seed:intended-local-console');
    expect(readScript(testsPackage, 'seed:intended-local-console')).toBe('node scripts/seed-intended-local-console.mjs');
    expect(readScript(rootPackage, 'check:intended-mutation-self-check')).toBe('pnpm -C tests check:intended-mutation-self-check');
    expect(readScript(testsPackage, 'check:intended-mutation-self-check')).toBe('node scripts/check-intended-mutation-self-check.mjs');
    expect(readScript(rootPackage, 'check:intended-mutation-self-check:complete')).toBe('pnpm -C tests check:intended-mutation-self-check:complete');
    expect(readScript(testsPackage, 'check:intended-mutation-self-check:complete')).toBe('node scripts/check-intended-mutation-self-check.mjs --require-detected');
    expect(readScript(rootPackage, 'preflight:intended-mutation-self-check')).toBe('pnpm -C tests preflight:intended-mutation-self-check');
    expect(readScript(testsPackage, 'preflight:intended-mutation-self-check')).toBe('pnpm run ensure:intended-google-token && node scripts/check-intended-mutation-self-check.mjs --preflight');
    expect(readScript(rootPackage, 'preflight:intended-mutation-self-check:ci')).toBe('pnpm -C tests preflight:intended-mutation-self-check:ci');
    expect(readScript(testsPackage, 'preflight:intended-mutation-self-check:ci')).toBe('pnpm run ensure:intended-google-token && node scripts/check-intended-mutation-self-check.mjs --preflight --ci');
});
test('Refactor 88 intended Google OIDC helper uses service-account impersonation', () => {
    ...
    expect(intendedConfigSource).toContain("dotenv.config({ path: path.join(repoRoot, '.env.intended.local'), override: true })");
    expect(preflightSource).toContain("dotenv.config({ path: path.join(repoRoot, '.env.intended.local'), override: true })");
    expect(startupSource).toContain("dotenv.config({ path: path.join(repoRoot, '.env.intended.local'), override: true })");
...
test('Refactor 88 registration-flow benchmark report stays marked historical', () => {
    ...
    const violations = [];
    if (!source.startsWith('# Registration Flow Benchmark Report\n\nStatus: archived historical report')) {
...
test('Refactor 88 signing integration suite stays off the fake relay server', () => {
    ...
    expect(integrationScript).toContain('playwright test -c playwright.integration.config.ts --reporter=line');
...
test('Refactor 88 intended harness fails fast when a public action click is inert', () => {
    ...
    const startMethod = source.match(/private async waitForIntendedPageActionStarted[\s\S]*?\n  private async closeExportViewerIfOpen/);
...
test('Refactor 88 intended harness only stubs external identity and chain RPC hosts', () => {
    const source = fs.readFileSync(intendedHarnessPath, 'utf8');
    const externalHosts = extractDelimitedSource(source, 'const EXTERNAL_HOST_PATTERNS = [', '] as const;');
    const routeHandler = extractDelimitedSource(source, 'private async handleExternalRoute', 'private handleConsoleMessage');
    const fulfillStub = extractDelimitedSource(source, 'async function fulfillExternalStub', 'async function fulfillNearRpcStub');
    ...
    assertTokensAppearInOrder(routeHandler, [
...
test('Refactor 88 key export actions use public exact-lane export UI', () => {
    ...
    expect(harnessSource).not.toMatch(/this\.passkeyPromptCount \+= 1;\s+this\.recordService\(`ed25519 export/);
});
test('Refactor 88 mutation self-check manifest covers known regression classes', () => {
    ...
    const uncheckedMutationProofRows = expectedMutationSelfCheckIds.filter((id) => planSource.includes(`- [ ] Run \`${id}\``));
    if (uncheckedMutationProofRows.length > 0) {
        expect(planSource).not.toContain('The Phase 3B mutation self-check has been run and every seeded regression');
    ...
    for (const mutation of mutations) {
        ...
        expect(requireStringField(mutation, 'expectedFailureOracle'), `${id} expectedFailureOracle`).toBeTruthy();
        ...
        for (const contractFile of targetedContracts) {
            expect(contractFiles.has(contractFile)).toBe(true);
        }
        ...
        for (const evidenceToken of evidenceTokens) {
            expect(harnessSource, `${id} missing harness evidence ${evidenceToken}`).toContain(evidenceToken);
        }
        const proof = requireRecordField(mutation, 'phase3bProof');
        const mutationId = id;
        expect(requireStringField(proof, 'status'), `${id} proof status`).toBe(expectedMutationProofStatuses[mutationId]);
        const expectedUnblockRequirement = expectedMutationUnblockRequirements[mutationId];
        if (expectedUnblockRequirement) {
            expect(requireStringField(proof, 'unblockRequirement'), `${id} unblockRequirement`).toBe(expectedUnblockRequirement);
        }
        else {
        ...
        expect(typeof proof.requiresEmailOtpGoogleIdToken, `${id} requiresEmailOtpGoogleIdToken`).toBe('boolean');
        if (id === 'first_post_step_up_transaction_failure') {
            expect(requireStringField(proof, 'observedFailureOracle')).toBe('post-step-up transaction failed');
        }
        const expectedDetectedEvidence = expectedMutationDetectedProofEvidence[mutationId];
        if (expectedDetectedEvidence) {
            expect(requireStringField(proof, 'observedAt'), `${id} observedAt`).toBe(expectedDetectedEvidence.observedAt);
            expect(requireStringField(proof, 'observedFailureCommand'), `${id} observedFailureCommand`).toBe(expectedDetectedEvidence.observedFailureCommand);
            expect(requireStringField(proof, 'restoredValidationCommand'), `${id} restoredValidationCommand`).toBe(expectedDetectedEvidence.restoredValidationCommand);
        }
        else {
        ...
        if (proof.requiresEmailOtpGoogleIdToken === true) {
            expect(localCommand, `${id} local token ensure`).toContain('pnpm -C tests run ensure:intended-google-token');
            expect(ciCommand, `${id} ci token ensure`).toContain('pnpm -C tests run ensure:intended-google-token');
        }
        expect(targetedContracts.some((contractFile) => localCommand.includes(contractFile) || ciCommand.includes(contractFile)), `${id} proof command target`).toBe(true);
    }
...
function allowsFakeAuthServiceSurface(relativePath) {
    return (relativePath.startsWith('tests/relayer/') ||
        fakeAuthServiceAllowedFiles.includes(relativePath));
}
...
function listSourceTextFilesRecursive(dir) {
    ...
    for (const entry of entries) {
        ...
        if (!entry.isFile() || !isSourceTextFile(entry.name))
            continue;
        files.push(path.relative(repoRoot, absolutePath).replaceAll(path.sep, '/'));
    }
    return files;
