...
test.describe('console organization access', () => {
  ...
  test('migration maps legacy access and drops team_members', async () => {
    const temporary = createTemporaryD1Database();
    try {
      const allMigrations = listD1MigrationFiles('d1-console');
      const legacyMigrations = allMigrations.filter((file) => !/\/002[0-9]_/u.test(file));
      const accessMigration = allMigrations.filter((file) =>
        /\/0020_console_organization_access\.sql$/u.test(file),
      );
      await applyD1MigrationFiles(temporary.database, legacyMigrations);
      const createdAtMs = Date.parse('2026-07-25T00:00:00.000Z');
      await temporary.database.exec(`
        INSERT INTO organizations
          (namespace, id, name, slug, created_by_user_id, status, created_at_ms, updated_at_ms)
        VALUES
          ('migration', 'org-migration', 'Migration Org', 'migration-org', 'legacy-owner', 'ACTIVE', ${createdAtMs}, ${createdAtMs});
        INSERT INTO projects
          (namespace, id, org_id, name, slug, status, created_at_ms, updated_at_ms)
        VALUES
          ('migration', 'project-migration', 'org-migration', 'Migration Project', 'migration-project', 'ACTIVE', ${createdAtMs}, ${createdAtMs});
        ...
        VALUES
          ('migration', 'legacy-owner-membership', 'org-migration', 'legacy-owner', 'owner@example.com', 'owner@example.com', 'Legacy Owner', 'ACTIVE', '[{"role":"owner","scope":"ORG"}]', 'legacy-owner', ${createdAtMs}, ${createdAtMs}, ${createdAtMs}, ${createdAtMs}),
          ('migration', 'legacy-admin-membership', 'org-migration', 'legacy-admin', 'admin@example.com', 'admin@example.com', 'Legacy Admin', 'ACTIVE', '[{"role":"admin","scope":"ORG"},{"role":"admin_manage_members","scope":"ORG"},{"role":"billing_write","scope":"ORG"}]', 'legacy-owner', ${createdAtMs}, ${createdAtMs}, ${createdAtMs}, ${createdAtMs}),
          ('migration', 'legacy-member-membership', 'org-migration', 'legacy-member', 'member@example.com', 'member@example.com', 'Legacy Member', 'ACTIVE', '[{"role":"overview_write","scope":"ORG"}]', 'legacy-owner', ${createdAtMs}, ${createdAtMs}, ${createdAtMs}, ${createdAtMs});
      `);
      await applyD1MigrationFiles(temporary.database, accessMigration);

      const organization = await temporary.database
        .prepare(
          `SELECT owner_anchor_membership_id, authorization_version
             FROM organizations
            WHERE namespace = 'migration'
              AND id = 'org-migration'`,
        )
        .first<Record<string, unknown>>();
      expect(organization?.owner_anchor_membership_id).toBe('legacy-owner-membership');
      expect(Number(organization?.authorization_version)).toBeGreaterThan(0);

      const memberships = await temporary.database
        .prepare(
          `SELECT id, role
             FROM organization_memberships
            WHERE namespace = 'migration'
              AND org_id = 'org-migration'
            ORDER BY id`,
        )
        .all<{ id?: unknown; role?: unknown }>();
      expect(memberships.results).toEqual([
        { id: 'legacy-admin-membership', role: 'ADMIN' },
        { id: 'legacy-member-membership', role: 'MEMBER' },
        { id: 'legacy-owner-membership', role: 'OWNER' },
      ]);
      const permissions = await temporary.database
        .prepare(
          `SELECT permission
             FROM organization_admin_permissions
            WHERE namespace = 'migration'
              AND membership_id = 'legacy-admin-membership'
            ORDER BY permission`,
        )
        .all<{ permission?: unknown }>();
      expect(permissions.results).toEqual([
        { permission: 'billing.manage' },
        { permission: 'billing.view' },
        { permission: 'members.manage' },
      ]);
      const projectAccess = await temporary.database
        .prepare(
          `SELECT access_level
             FROM project_member_access
            WHERE namespace = 'migration'
              AND membership_id = 'legacy-member-membership'`,
        )
        .first<{ access_level?: unknown }>();
      expect(projectAccess?.access_level).toBe('editor');
      const legacyTable = await temporary.database
        .prepare(
          `SELECT name
             FROM sqlite_master
            WHERE type = 'table'
              AND name = 'team_members'`,
        )
        .first();
