...
test.describe('relayer router (express) – P0', () => {
      ...
      executeSignedDelegate: async (input) => {
        ...
        return {
          ...
          outcome: {
            ...
            status: { SuccessValue: '' },
            transaction: { hash: 'delegate-tx-123' },
            transaction_outcome: {
              outcome: {
                gas_burnt: 1000,
                tokens_burnt: '250',
  ...
  test('POST /signed-delegate reserves and settles NEAR spend caps through the shared path', async () => {
    const service = makeFakeAuthService({
      executeSignedDelegate: async () => ({
        ok: true,
        transactionHash: 'delegate-tx-cap-123',
        outcome: {
          final_execution_status: 'FINAL',
          status: { SuccessValue: '' },
          transaction: { hash: 'delegate-tx-cap-123' },
          transaction_outcome: {
            outcome: {
              gas_burnt: 1000,
              tokens_burnt: '250',
  ...
  test('POST /signed-delegate excludes attached deposit from sponsored charge', async () => {
    ...
    const service = makeFakeAuthService({
      executeSignedDelegate: async () => ({
        ok: true,
        transactionHash: 'delegate-tx-deposit-123',
        outcome: {
          final_execution_status: 'FINAL',
          status: { SuccessValue: '' },
          transaction: { hash: 'delegate-tx-deposit-123' },
          transaction_outcome: {
            outcome: {
              gas_burnt: 1000,
              tokens_burnt: '250',
  ...
  test('POST /wallet/email-otp/dev/cleanup-google-registration verifies Google token before cleanup', async () => {
    ...
    try {
      const res = await fetchJson(
        `${srv.baseUrl}/wallet/email-otp/dev/cleanup-google-registration`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
    ...
    const session = makeSessionAdapter({
      ...
      parse: async (headers) => {
        ...
        const token = cookieHeader
          .split(';')
          .map((part) => part.trim())
          .find((part) => part.startsWith('seams-jwt='))
          ?.slice('seams-jwt='.length);
  ...
  test('POST /session/exchange: passkey_assertion + sessionKind=cookie sets cookie and omits jwt', async () => {
    ...
    try {
      ...
      expect(res.json?.jwt).toBeUndefined();
      expect(res.headers.get('set-cookie')).toContain('seams-jwt=app-jwt-passkey-cookie-1');
    } finally {
      await srv.close();
    }
  });
  ...
  test('POST /session/refresh: cookie session sets Set-Cookie and returns { ok: true }', async () => {
    ...
    try {
      ...
      expect(res.status).toBe(200);
      expect(res.headers.get('set-cookie')).toContain('seams-jwt=refreshed-999');
      expect(res.json?.ok).toBe(true);
      expect(res.json?.jwt).toBeUndefined();
    } finally {
      await srv.close();
      ...
      signingSessionSeal: {
        ...
        sessionPolicy: {
          ...
          getThresholdSessionStatuses: async ({ thresholdSessionId }) =>
            thresholdSessionId === claims.thresholdSessionId ? [wrongCurveStatus, ecdsaStatus] : [],
        },
      },
    });
    const srv = await startExpressRouter(router);
      ...
      signingSessionSeal: {
        ...
        sessionPolicy: {
          ...
          getWalletBudgetStatus: async ({ signingGrantId }) =>
            signingGrantId === claims.signingGrantId ? walletBudgetStatus : null,
          getThresholdSessionStatuses: async ({ thresholdSessionId }) =>
            thresholdSessionId === claims.thresholdSessionId ? [ecdsaStatus] : [],
        },
      },
    });
    const srv = await startExpressRouter(router);
  ...
  test('POST /session/signing-budget/status: mismatched threshold session is rejected', async () => {
    ...
    try {
      const res = await fetchJson(`${srv.baseUrl}/session/signing-budget/status`, {
        ...
        body: JSON.stringify({
          signingGrantId: claims.signingGrantId,
          thresholdSessionId: `${claims.thresholdSessionId}-other`,
        }),
      });
  ...
  test('POST /signed-delegate replays an existing successful delegate execution by idempotency key', async () => {
    let executeCalls = 0;
    const service = makeFakeAuthService({
      executeSignedDelegate: async () => {
        executeCalls += 1;
        return {
          ok: true,
          transactionHash: 'delegate-tx-replay-123',
          outcome: {
            final_execution_status: 'FINAL',
            status: { SuccessValue: '' },
            transaction: { hash: 'delegate-tx-replay-123' },
            transaction_outcome: {
              outcome: {
                gas_burnt: 1000,
                tokens_burnt: '250',
    ...
    try {
      ...
      const first = await fetchJson(`${srv.baseUrl}/signed-delegate`, request);
      const second = await fetchJson(`${srv.baseUrl}/signed-delegate`, request);
