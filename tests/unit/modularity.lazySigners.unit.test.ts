...
test.describe('modularity lazy signer loading', () => {
  ...
  test('near adapter path does not instantiate multichain wasm workers', async ({ page }) => {
    const result = await page.evaluate(
      async ({ paths }) => {
        ...
        const countMultichainWorkers = (
          entries: Array<{ url: string; name: string | null }>,
        ): number => entries.filter((entry) => isEthWorker(entry) || isTempoWorker(entry)).length;
        ...
        try {
          ...
          return {
            multichainWorkerDelta:
              countMultichainWorkers(workerCreations) - baselineMultichainCount,
            workerCreations,
          };
        } finally {
