...
const IMPORT_PATHS = {
  nearBuilder:
    '/sdk/esm/core/signingEngine/touchConfirm/displayFormat/nearTx.js',
  evmBuilder:
    '/sdk/esm/core/signingEngine/touchConfirm/displayFormat/evmTx.js',
  tempoBuilder:
    '/sdk/esm/core/signingEngine/touchConfirm/displayFormat/tempoTx.js',
  txTreeUtils:
    '/sdk/esm/core/signingEngine/touchConfirm/ui/lit-components/TxTree/tx-tree-utils.js',
} as const;
...
test.describe('touchConfirm display model fixtures', () => {
  ...
  test('decodes ERC-4337 handleOps + execute callData into child calls', async ({ page }) => {
    ...
    const model = await page.evaluate(async ({ paths, entryPointArg, handleOpsDataArg }) => {
  ...
  test('decodes executeBatch callData for direct smart-account calls', async ({ page }) => {
    ...
    const model = await page.evaluate(async ({ paths, smartAccountArg, executeBatchDataArg }) => {
      const { buildEvmDisplayModel } = await import(paths.evmBuilder);
      return buildEvmDisplayModel({
        request: {
          chain: 'evm',
  ...
  test('falls back safely for partial ERC-4337 decode failures', async ({ page }) => {
    ...
    const model = await page.evaluate(async ({ paths, malformedData }) => {
      const { buildEvmDisplayModel } = await import(paths.evmBuilder);
      return buildEvmDisplayModel({
        request: {
          chain: 'evm',
  ...
  test('builds review tree from model without txSigningRequests', async ({ page }) => {
    const result = await page.evaluate(async ({ paths }) => {
      ...
      const hasTree = !!treeNode && Array.isArray(treeNode.children) && treeNode.children.length > 0;
