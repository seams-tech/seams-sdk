...
test.describe('TatchiPasskey namespaced signing surface', () => {
  ...
  test('TatchiPasskey exposes near/tempo/evm namespaces without flat root signing methods', async ({ page }) => {
    const result = await page.evaluate(async ({ flatMethods }) => {
