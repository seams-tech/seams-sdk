...
const IMPORT_PATHS = {
  privateKeyExportRecovery:
    '/sdk/esm/core/signingEngine/api/recovery/privateKeyExportRecovery.js',
} as const;
...
test.describe('private key export recovery hardening', () => {
  ...
  test('fails closed with typed error + telemetry when export worker op is unavailable', async ({ page }) => {
    const result = await page.evaluate(async ({ paths }) => {
      ...
      try {
        ...
        const telemetry = warnings.find((entry) => entry[0] === '[signer-export-telemetry]') || [];
        return {
          ok: false,
  ...
  test('fails closed with typed error + telemetry on legacy worker message mismatch', async ({ page }) => {
    const result = await page.evaluate(async ({ paths }) => {
      const mod = await import(paths.privateKeyExportRecovery);
      const warnings: unknown[][] = [];
      const originalWarn = console.warn;
