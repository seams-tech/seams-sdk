...
test('authenticates the worker delivery recipient while creating target preparation', async () => {
  temporary = await openDatabase();
  const fixture = buildR103DeviceLinkFixture({ linkSessionId: 'link-session:route-preparation' });
  ...
  let receivedRecipient: string | undefined;
  const routeService: DeviceLinkingRouteServiceV1 = {
    ...
    targetCredential: {
      ...
      getTargetPreparationV1: async (input) => {
        ...
        receivedRecipient = input.deliveryRecipientPublicKey65B64u;
        return preparation;
      },
    },
  };
  ...
  expect(await response.json()).toEqual(preparation);
