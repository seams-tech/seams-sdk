...
test.describe('encodeSignedTransactionBase64', () => {
  ...
  test('encodes SignedTransaction instances via methods', async ({ page }) => {
    const res = await page.evaluate(
      async ({ paths }) => {
        try {
          const { SignedTransaction, encodeSignedTransactionBase64 } = await import(
            paths.nearClient
          );
    ...
    if (!res.success) {
      test.skip(
        true,
        `encodeSignedTransactionBase64 (methods) skipped: ${res.error || 'unknown error'}`,
      );
      return;
    }
  ...
  test('encodes plain objects with borsh_bytes array', async ({ page }) => {
    ...
    if (!res.success) {
      test.skip(
        true,
        `encodeSignedTransactionBase64 (borsh_bytes) skipped: ${res.error || 'unknown error'}`,
      );
      return;
    }
  ...
  test('encodes plain objects with borshBytes typed array', async ({ page }) => {
    ...
    if (!res.success) {
      test.skip(
        true,
        `encodeSignedTransactionBase64 (borshBytes) skipped: ${res.error || 'unknown error'}`,
      );
      return;
    }
  ...
  test('throws on invalid payloads with no bytes', async ({ page }) => {
    ...
    if (!res.success) {
      test.skip(
        true,
        `encodeSignedTransactionBase64 (invalid payload) skipped: ${res.error || 'unknown error'}`,
      );
      return;
    }
  ...
  test('handles large borsh_bytes payloads without stack overflow', async ({ page }) => {
        ...
        try {
          ...
          for (const idx of sampleIndices) {
            ...
            if (expectedByte !== actualByte) {
              return {
                success: false,
                error: `Byte mismatch at ${idx}: ${actualByte} != ${expectedByte}`,
              };
            }
          }
    ...
    if (!res.success) {
      test.skip(
        true,
        `encodeSignedTransactionBase64 (large payload) skipped: ${res.error || 'unknown error'}`,
      );
      return;
    }
