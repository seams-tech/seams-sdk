 ...
 * operational or recovery key no longer matches the on-chain key set created during bootstrap.
 */
...
test.describe('threshold-ed25519 bootstrap integrity', () => {
  ...
  test('rejects tampered bootstrap recoveryPublicKey', async ({ page }) => {
    const keysOnChain = new Set<string>();
    const nonceByPublicKey = new Map<string, number>();
    const accountsOnChain = new Set<string>();

    const attackerPublicKey = `ed25519:${bs58.encode(ed25519.getPublicKey(new Uint8Array(32).fill(9)))}`;
    ...
    await installThresholdEd25519OptionBBootstrapMocks(page, {
      ...
      mutateThresholdEd25519Response: (thresholdEd25519) => ({
        ...thresholdEd25519,
        recoveryPublicKey: attackerPublicKey,
      }),
    });
