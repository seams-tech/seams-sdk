  ...
  test('fails fast when exhausted and succeeds after explicit login reconnect', async ({
    ...
    try {
      ...
      const result = (await page.evaluate(
        async ({ relayerUrl }) => {
          ...
          try {
            ...
            if (
              !/(threshold signingSession is (not_found|exhausted|expired)|threshold signingSession auth is unavailable)/i.test(
      ...
      expect(result.secondFailureMessage).toMatch(
        /(threshold signingSession is (not_found|exhausted|expired)|threshold signingSession auth is unavailable)/i,
      );
