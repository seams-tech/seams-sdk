    ...
    try {
      const result = await page.evaluate(async ({ relayerUrl }) => {
        try {
          ...
          const lastProfileState = await IndexedDBManager.clientDB.getLastProfileState().catch(() => null);
          ...
          const linkedAccounts = (await IndexedDBManager.clientDB.listChainAccountsByProfile(profileId))
            .map((row: any) => {
              const chainIdKey = String(row?.chainIdKey || '').trim().toLowerCase();
          ...
          const manualBootstrap = {
            ...
            clientAdditiveShare32B64u: String(
              bootstrap.keygen?.clientAdditiveShare32B64u || '',
            ),
            relayerKeyId: String(bootstrap.keygen?.relayerKeyId || ''),
            thresholdEcdsaPublicKeyB64u: String(bootstrap.keygen?.thresholdEcdsaPublicKeyB64u || ''),
