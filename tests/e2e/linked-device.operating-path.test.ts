...
async function openWallet(page: Page): Promise<void> {
  ...
  await page.locator('iframe[src*="/wallet-service"]').waitFor({ state: 'attached', timeout: 30_000 });
}
...
async function registerOwner(page: Page): Promise<void> {
  ...
  await primary.click();
  await page.locator('.w3a-profile-button-morphable').waitFor({ state: 'visible', timeout: 120_000 });
}
...
async function openDevice2Qr(page: Page): Promise<string> {
  await openWallet(page);
  const linkButton = page.getByRole('button', { name: 'Scan and Link Device', exact: true });
  await linkButton.waitFor({ state: 'visible', timeout: 30_000 });
  await linkButton.click();
  const qr = page.locator('img[alt="Device Linking QR Code"]');
...
test('Device 2 QR → Device 1 scan → Wallet Session → linked signing → revocation', async ({
  ...
  try {
    ...
    await device2Page.getByText('Linked device active', { exact: true }).waitFor({
      state: 'visible',
      timeout: 180_000,
    });
    ...
    await ownerPage.locator('.w3a-linked-devices-row').waitFor({ state: 'visible', timeout: 60_000 });
