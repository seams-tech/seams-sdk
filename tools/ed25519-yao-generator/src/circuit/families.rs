...
pub fn compile_provisional_activation_core_v1() -> ProvisionalActivationCoreV1 {
    ...
    let a_y_client = field_bits(&inputs, 0);
    let a_y_server = field_bits(&inputs, 1);
    let a_tau_client = field_bits(&inputs, 2);
    let a_tau_server = field_bits(&inputs, 3);
    let b_y_client = field_bits(&inputs, 4);
    let b_y_server = field_bits(&inputs, 5);
    let b_tau_client = field_bits(&inputs, 6);
    let b_tau_server = field_bits(&inputs, 7);

    let y_a = wrapping_add_le_256_bits(&mut builder, a_y_client, a_y_server);
    let y_b = wrapping_add_le_256_bits(&mut builder, b_y_client, b_y_server);
    let seed = wrapping_add_le_256_bits(&mut builder, y_a, y_b);
    let digest = sha512_fixed_32_bits(&mut builder, seed);
    ...
    let signing_scalar = reduce_clamped_mod_l_bits(&mut builder, clamped);

    let tau_a = add_mod_l_bits(&mut builder, a_tau_client, a_tau_server);
    let tau_b = add_mod_l_bits(&mut builder, b_tau_client, b_tau_server);
    let tau = add_mod_l_bits(&mut builder, tau_a, tau_b);
    let x_client_base = add_mod_l_bits(&mut builder, signing_scalar, tau);
    let x_server_base = add_mod_l_bits(&mut builder, x_client_base, tau);
...
pub fn compile_provisional_export_core_v1() -> ProvisionalExportCoreV1 {
    ...
    let a_y_client = field_bits(&inputs, 0);
    let a_y_server = field_bits(&inputs, 1);
    let b_y_client = field_bits(&inputs, 2);
    let b_y_server = field_bits(&inputs, 3);
    let y_a = wrapping_add_le_256_bits(&mut builder, a_y_client, a_y_server);
    let y_b = wrapping_add_le_256_bits(&mut builder, b_y_client, b_y_server);
    let seed = wrapping_add_le_256_bits(&mut builder, y_a, y_b);
    let circuit = builder
...
fn field_bits(inputs: &[BuilderBit], field_index: usize) -> [BuilderBit; FIELD_BIT_WIDTH] {
...
fn activation_input_bits(inputs: &PublicSyntheticActivationCoreInputsV1) -> Vec<bool> {
    let mut bits = Vec::with_capacity(ACTIVATION_INPUT_BIT_WIDTH as usize);
    append_lsb0_bits(&mut bits, inputs.deriver_a.0.y_client);
    append_lsb0_bits(&mut bits, inputs.deriver_a.0.y_server);
    append_lsb0_bits(&mut bits, inputs.deriver_a.0.tau_client);
    append_lsb0_bits(&mut bits, inputs.deriver_a.0.tau_server);
    append_lsb0_bits(&mut bits, inputs.deriver_b.0.y_client);
    append_lsb0_bits(&mut bits, inputs.deriver_b.0.y_server);
    append_lsb0_bits(&mut bits, inputs.deriver_b.0.tau_client);
    append_lsb0_bits(&mut bits, inputs.deriver_b.0.tau_server);
    bits
}

fn export_input_bits(inputs: &PublicSyntheticExportCoreInputsV1) -> Vec<bool> {
    let mut bits = Vec::with_capacity(EXPORT_INPUT_BIT_WIDTH as usize);
    append_lsb0_bits(&mut bits, inputs.deriver_a.0.y_client);
    append_lsb0_bits(&mut bits, inputs.deriver_a.0.y_server);
    append_lsb0_bits(&mut bits, inputs.deriver_b.0.y_client);
    append_lsb0_bits(&mut bits, inputs.deriver_b.0.y_server);
...
fn append_lsb0_bits(output: &mut Vec<bool>, bytes: [u8; 32]) {
    for byte in bytes {
        for bit_index in 0..8 {
            output.push(((byte >> bit_index) & 1) == 1);
        }
    }
}

fn lsb0_bits_to_32_bytes(bits: &[bool]) -> [u8; 32] {
    assert_eq!(
        bits.len(),
        FIELD_BIT_WIDTH,
