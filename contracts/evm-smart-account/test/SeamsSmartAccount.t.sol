...
contract SeamsSmartAccountTest is TestBase {
  ...
  bytes32 private constant RECOVERY_NAME_HASH = keccak256("SeamsSmartAccountRecovery");
  ...
  SeamsSmartAccount private account;
  ...
  function testInitializeSetsStateAndRejectsSecondRun() public {
    ...
    vm.expectRevert(abi.encodeWithSelector(SeamsSmartAccount.AlreadyInitialized.selector));
    account.initialize(
      SmartAccountInit({
  ...
  function testInitializeRejectsInvalidInputs() public {
    SeamsSmartAccount invalidNearAccount = new SeamsSmartAccount();
    address[] memory owners = new address[](1);
    owners[0] = ownerOne;

    vm.expectRevert(
      abi.encodeWithSelector(SeamsSmartAccount.InvalidNearAccountBinding.selector)
    );
    ...
    SeamsSmartAccount invalidAuthorityAccount = new SeamsSmartAccount();
    vm.expectRevert(
      abi.encodeWithSelector(SeamsSmartAccount.InvalidRecoveryAuthority.selector)
    );
    ...
    SeamsSmartAccount emptyOwnersAccount = new SeamsSmartAccount();
    address[] memory noOwners = new address[](0);
    vm.expectRevert(abi.encodeWithSelector(SeamsSmartAccount.InvalidOwner.selector));
    ...
    SeamsSmartAccount zeroOwnerAccount = new SeamsSmartAccount();
    ...
    SeamsSmartAccount duplicateOwnerAccount = new SeamsSmartAccount();
  ...
  function testOwnerManagementAllowsOwnerAndEntryPoint() public {
    ...
    vm.expectRevert(abi.encodeWithSelector(SeamsSmartAccount.NotOwner.selector));
  ...
  function testRemoveOwnerProtectsLastOwner() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(SeamsSmartAccount.LastOwnerRemovalForbidden.selector)
    );
    vm.prank(ownerOne);
  ...
  function testExecuteAndBatchExecution() public {
    ...
    vm.expectRevert(abi.encodeWithSelector(SeamsSmartAccount.ArrayLengthMismatch.selector));
    vm.prank(ownerOne);
    account.executeBatch(targets, values, shortData);
  }

  function testExecutionRejectsZeroAddressTargets() public {
    vm.expectRevert(abi.encodeWithSelector(SeamsSmartAccount.InvalidExecutionTarget.selector));
    vm.prank(ownerOne);
  ...
  function testVerifyAndRecoverIsPublicAndSharesReplayStore() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        SeamsSmartAccount.RecoveryNonceAlreadyUsed.selector, bytes32(nonce)
      )
    );
  ...
  function testRecoveryRejectsBadBindingAndExpiredAuthorization() public {
    ...
    vm.expectRevert(abi.encodeWithSelector(SeamsSmartAccount.InvalidNearAccountBinding.selector));
    account.verifyAndRecover(
      wrongNearHash,
      newNearKeyHash,
      ownerThree,
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        SeamsSmartAccount.RecoveryAuthorizationExpired.selector,
        block.timestamp - 1,
        block.timestamp
      )
    );
  ...
  function testRecoveryRejectsBadSignerAndDuplicateOwner() public {
    ...
    vm.expectRevert(abi.encodeWithSelector(SeamsSmartAccount.InvalidRecoverySignature.selector));
    account.verifyAndRecover(
      nearHash,
      newNearKeyHash,
      ownerThree,
    ...
    vm.expectRevert(
      abi.encodeWithSelector(SeamsSmartAccount.OwnerAlreadyExists.selector, ownerTwo)
    );
    account.verifyAndRecover(
      nearHash,
      newNearKeyHash,
  ...
  function testRecoveryAuthorizationCannotReplayAcrossDifferentAccounts() public {
    SeamsSmartAccount secondAccount = new SeamsSmartAccount();
    address[] memory owners = new address[](2);
    owners[0] = ownerOne;
    owners[1] = ownerTwo;
  ...
  function testRecoveredOwnerCannotBeRemovedByLegacyOwners() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(SeamsSmartAccount.RecoveredOwnerProtected.selector, ownerThree)
    );
    vm.prank(ownerOne);
    account.removeOwner(ownerThree);
  ...
  function testValidateUserOpRejectsUnexpectedCallerAndBadSignature() public {
    ...
    vm.expectRevert(abi.encodeWithSelector(SeamsSmartAccount.InvalidEntryPoint.selector));
  ...
  function testValidateUserOpRejectsSenderMismatch() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(SeamsSmartAccount.InvalidUserOpSender.selector, userOp.sender)
    );
    vm.prank(entryPoint);
    account.validateUserOp(userOp, userOpHash, 0);
  }
