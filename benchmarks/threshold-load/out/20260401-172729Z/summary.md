# Threshold Load Report

Generated: 2026-04-01T17:27:29.553Z
Run ID: `20260401-172729Z`

## Scenario Summary

| Scenario | Description | Status | Success Rate | Signs/sec | Sign p95 (ms) |
|---|---|---|---:|---:|---:|
| `ed25519_local_burst_smoke` | Threshold Ed25519 local warm-session synchronized burst smoke profile | ok | 100.0% | 219.84 | 33.45 |

## ed25519_local_burst_smoke

- Description: Threshold Ed25519 local warm-session synchronized burst smoke profile
- Status: ok
- Command: `pnpm exec tsx --tsconfig ./client/tsconfig.json ./benchmarks/threshold-load/src/scenario-harness.ts --scenario ed25519_local_burst --wallets 8 --signs-per-wallet 1 --max-concurrency 8 --profile burst`
- Profile: burst
- Wallets: 8
- Signs per wallet: 1
- Max concurrency: 8
- Bootstrap duration (ms): 44.92
- Signing duration (ms): 36.39
- Total attempts: 8
- Total success: 8
- Total failure: 0
- Success rate: 100.0%
- Throughput (signs/sec): 219.84

#### Bootstrap Session Mint

| Metric | Count | p50 | p95 | p99 | Mean | Max |
|---|---:|---:|---:|---:|---:|---:|
| Bootstrap Session Mint | 8 | 21.27 | 23.37 | 23.37 | 21.78 | 23.37 |

#### End-to-End Sign

| Metric | Count | p50 | p95 | p99 | Mean | Max |
|---|---:|---:|---:|---:|---:|---:|
| End-to-End Sign | 8 | 26.13 | 33.45 | 33.45 | 27.69 | 33.45 |

### Bootstrap Routes

| Route | Count | p50 (ms) | p95 (ms) | p99 (ms) | Mean (ms) | Max (ms) |
|---|---:|---:|---:|---:|---:|---:|
| `/threshold-ed25519/session` | 8 | 21.27 | 23.37 | 23.37 | 21.78 | 23.37 |

### Signing Routes

| Route | Count | p50 (ms) | p95 (ms) | p99 (ms) | Mean (ms) | Max (ms) |
|---|---:|---:|---:|---:|---:|---:|
| `/threshold-ed25519/authorize` | 8 | 9.75 | 14.31 | 14.31 | 10.32 | 14.31 |
| `/threshold-ed25519/sign/finalize` | 8 | 3.01 | 6.21 | 6.21 | 3.64 | 6.21 |
| `/threshold-ed25519/sign/init` | 8 | 11.39 | 20.63 | 20.63 | 12.27 | 20.63 |

### System

| Metric | Value |
|---|---:|
| cpuUserMsTotal | 64.95 |
| cpuSystemMsTotal | 2.98 |
| rssMb p95 | n/a |
| rssMb max | n/a |
| heapUsedMb p95 | n/a |
| heapUsedMb max | n/a |
| eventLoopDelayMs p95 | n/a |
| eventLoopDelayMs max | n/a |

## Notes

- Current coverage is threshold-ed25519 warm-session local 2-party only.
- The actor provisions canonical single-key material directly, then measures the kept warm signing path.
- ECDSA, multi-node routing, backend comparison, and relayer-cosigner topologies remain follow-on work.
