...
export function resolveWarmEcdsaBootstrapRequestFromSession(args: {
  ...
  const preferredSessionKind =
    ...
    'jwt';
  // Fresh WebAuthn bootstrap must mint fresh server session/budget identity.
  const reusableSessionId = shouldUseFreshWebAuthnBootstrap
    ? undefined
    : toOptionalNonEmptyString(reusableWarmCapability?.record?.thresholdSessionId);
  const reusableWalletSigningSessionId = shouldUseFreshWebAuthnBootstrap
    ? undefined
  ...
  return {
    ...
    ...(explicitWalletSigningSessionId
      ...
      : reusableWalletSigningSessionId
        ? {
