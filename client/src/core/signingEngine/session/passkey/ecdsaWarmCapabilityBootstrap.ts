  ...
  return await provisionWarmEcdsaCapability(
    {
      ...
      claimPrfFirstByThresholdSessionId: (claimArgs) => {
        if (claimArgs.authMethod !== 'passkey') {
          throw new Error(
            '[WarmSessionStore] passkey ECDSA bootstrap requires passkey auth to claim warm-session PRF',
          );
        }
        ...
        if (!walletSigningSessionId) {
          throw new Error(
            '[WarmSessionStore] passkey ECDSA bootstrap requires walletSigningSessionId for warm-session restore',
          );
        }
