...
export type EvmFamilyEcdsaIdentityMismatch =
  ...
  | ({ kind: 'subject_mismatch'; field: 'subjectId' } & IdentityMismatchDetails)
...
export type ResolveReadyEvmFamilyEcdsaMaterialInput = {
  ...
  expected: {
    ...
    subjectId: WalletSubjectId | string;
    chainTarget: ThresholdEcdsaChainTarget;
    authMethod: EvmFamilyEcdsaAuthMethod;
    source: ThresholdEcdsaSessionStoreSource;
...
function normalizeThresholdSessionAuthToken(args: {
  ...
  if (!token) throw new Error('[evm-family-ecdsa] thresholdSessionAuthToken is required for jwt sessions');
...
function firstKeyMismatch(
  ...
  if (String(left.subjectId) !== String(right.subjectId)) {
    return mismatch('subject_mismatch', 'subjectId', left.subjectId, right.subjectId);
  }
...
export function buildEvmFamilyEcdsaKeyIdentityFromRecord(args: {
  ...
  return buildEvmFamilyEcdsaKeyIdentity({
...
export function buildEvmFamilyEcdsaKeyIdentityFromKeyRef(args: {
  ...
  return buildEvmFamilyEcdsaKeyIdentity({
    ...
    subjectId: args.keyRef.subjectId,
    rpId: args.rpId,
...
export function resolveReadyEvmFamilyEcdsaMaterial(
  ...
  const expectedSubjectId = toWalletSubjectId(input.expected.subjectId);
  if (String(recordKey.subjectId) !== String(expectedSubjectId)) {
    return {
      kind: 'identity_mismatch',
      reason: mismatch('subject_mismatch', 'subjectId', expectedSubjectId, recordKey.subjectId),
    };
  }
