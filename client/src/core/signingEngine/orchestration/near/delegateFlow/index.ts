...
} from '@/core/signingEngine/threshold/session/thresholdEd25519AuthSession';
import {
...
import {
  ensureEd25519Prefix,
  toPublicKeyString,
} from '@/core/signingEngine/workers/signerWorkerManager/internal/validation';
import { deriveThresholdEd25519ClientVerifyingShare } from '@/core/signingEngine/workers/signerWorkerManager/nearKeyOps/deriveThresholdEd25519ClientVerifyingShare';
import { executeSignerWorkerOperation } from '@/core/signingEngine/workers/operations/executeSignerWorkerOperation';
import {
  generateSessionId,
...
import {
  buildThresholdSessionPolicyForNearSigning,
...
export async function signDelegateAction({
  ...
  if (signingContext.threshold && signingAuthMode === 'warmSession') {
    ...
    if (delivered.ok) {
      ...
      thresholdSessionPlan = await buildThresholdSessionPolicyForNearSigning({
        nearAccountId,
        getRpId: () => ctx.touchIdPrompt.getRpId(),
