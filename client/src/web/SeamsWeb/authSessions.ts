...
export async function unlockDomain(
  ...
  if (result?.success) {
    ...
    await deps.getContext().signingRuntime.services.registrationAccounts.initializeCurrentUser({
...
export async function hasPasskeyCredentialDomain(
  ...
  return await deps.getContext().signingRuntime.services.registrationAccounts.hasPasskeyCredential(
    baseAccountId,
  );
}
