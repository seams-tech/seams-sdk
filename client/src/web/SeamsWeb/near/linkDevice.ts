...
export class LinkDeviceFlow {
  private context: SeamsWebContext;
  ...
  constructor(context: SeamsWebContext, options: StartDevice2LinkingFlowArgs) {
  ...
  private async completeLinking(): Promise<void> {
    ...
    const signed =
      await this.context.signingRuntime.services.nearKeyOperations.signTransactionWithKeyPair({
    ...
    if (ecdsaPrepare && this.session?.sessionId) {
      ...
      const preparedClientBootstrap =
        await this.context.signingRuntime.services.ecdsaRegistrationBootstrap.preparePasskeyClientBootstrap({
      ...
      await this.context.signingRuntime.services.ecdsaWalletRecords.storeWalletEcdsaSignerRecords({
  ...
  async generateQR(): Promise<{ qrData: DeviceLinkingQRData; qrCodeDataURL: string }> {
    try {
      ...
      const tempKeypair =
        await this.context.signingRuntime.services.nearKeyOperations.generateEphemeralNearKeypair();
  ...
  private async storeDeviceAuthenticator(
    ...
    const registrationAccounts = this.context.signingRuntime.services.registrationAccounts;

    // 1) Store user data first (also sets last-user/profile pointer).
    await registrationAccounts.storeUserData({
      nearAccountId,
      signerSlot,
    ...
    await registrationAccounts.storeAuthenticator({
      nearAccountId,
      credentialId,
      credentialPublicKey,
...
export class DeviceLinkingDomain {
  private readonly getContext: () => SeamsWebContext;
