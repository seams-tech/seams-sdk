...
type PersistRegisteredThresholdEd25519SessionBaseArgs = {
  ...
  signingRuntime: SeamsWebContext['signingRuntime'];
...
export async function persistRegisteredThresholdEd25519Session(
  ...
  await args.signingRuntime.services.warmSessions.hydrateSigningSession({
    sessionId,
    prfFirstB64u,
    expiresAtMs,
    remainingUses,
...
export async function hydrateThresholdWarmSessionFromRelay(args: {
  ...
  await args.context.signingRuntime.services.warmSessions.hydrateSigningSession({
    sessionId,
    prfFirstB64u,
    expiresAtMs: Math.floor(expiresAtMs),
    remainingUses,
