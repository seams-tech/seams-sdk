...
async function readLoginUnlockAccountPhase(args: {
  ...
  const [hintUser, lastUser, latestByAccount, authenticators] = await Promise.all([
    hintUserPromise,
    args.registrationAccounts.getLastUser().catch(() => null),
    getNearAccountProjection(IndexedDBManager, args.nearAccountId).catch(() => null),
    args.registrationAccounts.nearAuthenticatorsByAccount(args.nearAccountId).catch(() => []),
  ]);
  ...
  if (hintUser && hintUser.nearAccountId === args.nearAccountId) {
    ...
    userData = await args.registrationAccounts
...
export async function unlock(
  ...
  try {
    ...
    const accountPhase = await readLoginUnlockAccountPhase({
      registrationAccounts: context.signingRuntime.services.registrationAccounts,
      nearAccountId,
      signerSlotHint,
      selection: walletUnlockSelection,
      onEvent,
...
async function primeThresholdLoginWarmSigners(args: {
  context: SeamsWebContext;
  signingEngine: SeamsWebContext['signingEngine'];
  nearAccountId: AccountId;
  relayerUrl: string;
  relayerKeyId: string;
...
function isThresholdSignerMode(context: SeamsWebContext): boolean {
...
async function getLoginStateInternal(
  ...
  const registrationAccounts = context.signingRuntime.services.registrationAccounts;
  try {
    const lastUser = await registrationAccounts.getLastUser().catch(() => null);
    ...
    const userData =
      ...
      (await registrationAccounts.getUserBySignerSlot(targetAccountId, 1).catch(() => null));
...
export async function getRecentUnlocks(
  ...
  const registrationAccounts = context.signingRuntime.services.registrationAccounts;
  const allUsersData = await registrationAccounts.getAllUsers();
  const accountIds = [...new Set(allUsersData.map((user) => user.nearAccountId))];
  const lastUsedAccount = await registrationAccounts.getLastUser();
  return {
    accountIds,
 ...
 */
export async function lock(context: SeamsWebContext): Promise<void> {
  const { signingEngine } = context;
