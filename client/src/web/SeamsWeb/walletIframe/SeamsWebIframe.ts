...
import type { SignNEP413MessageParams, SignNEP413MessageResult } from '@/web/SeamsWeb/operations/near';
...
} from '@/web/SeamsWeb/operations/authMethods/emailOtp/recoveryCodeBackup';
...
export class SeamsWebIframe {
  ...
  constructor(configs: SeamsConfigsInput) {
    ...
    this.auth = {
      ...
      prefillThresholdEcdsaPresignPool: async (args) =>
        await this.prefillThresholdEcdsaPresignPoolDomain(args),
  ...
  private async prefillThresholdEcdsaPresignPoolDomain(args: {
  ...
  private async sendDelegateActionViaRelayerDomain(args: {
    ...
    const { sendDelegateActionViaRelayer } = await import('@/web/SeamsWeb/operations/near');
