...
- Host posts PROGRESS from `onEvent`:
  - `src/web/SeamsWeb/walletIframe/host/wallet-iframe-handlers.ts`
...
- Message types:
  - `src/web/SeamsWeb/walletIframe/shared/messages.ts` (`ProgressPayload`, PROGRESS/PM_RESULT/ERROR envelopes, `options.sticky`)
- Overlay behavior:
  - `src/web/SeamsWeb/walletIframe/client/progress/on-events-progress-bus.ts`
