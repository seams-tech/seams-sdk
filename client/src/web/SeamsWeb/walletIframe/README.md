...
- Uses class-based styling from `client/src/web/SeamsWeb/walletIframe/client/overlay/overlay-styles.ts` (no inline styles).
...
- Initial mount (hidden):
  - `client/src/web/SeamsWeb/walletIframe/client/transport/IframeTransport.ts` mounts the iframe with `w3a-wallet-overlay is-hidden`, `width/height: 0`, `aria-hidden`, and `tabindex=-1` so it is invisible yet present in the DOM. Base styles come from `client/src/web/SeamsWeb/walletIframe/client/overlay/overlay-styles.ts`.

- Expand to full‑screen during activation:
  - `showFrameForActivation()` in `client/src/web/SeamsWeb/walletIframe/client/router.ts` ensures the iframe exists and delegates to `OverlayController.showFullscreen()`, which applies the fullscreen class (fixed inset, pointer-events enabled, z-index 2147483646).
...
- When the overlay shows/hides automatically:
  - `client/src/web/SeamsWeb/walletIframe/client/progress/on-events-progress-bus.ts` implements `defaultOverlayIntentResolver`, which reads v2 `WalletFlowEvent.interaction.overlay`.
