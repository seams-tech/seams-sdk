...
import type { VoiceIdVerifier, VoiceIdEnrollmentEmbedding } from './verifier/VoiceIdVerifier.ts';
...
export class VoiceIdService {
  ...
  async startEnrollment(input: {
    ...
    this.dependencies.emitAuditEvent(this.audit('enrollment_started', record, null, 'issued'));
  ...
  async addEnrollmentSample(
    ...
    const embedding = await this.dependencies.verifier.extractEnrollmentEmbedding({ audio: sample.audio });
    ...
    this.dependencies.emitAuditEvent(this.audit('enrollment_sample_recorded', updated, null, embedding.quality.kind));
  ...
  async finalizeEnrollment(input: {
    ...
    const template = await this.dependencies.verifier.buildTemplate({
    ...
    this.dependencies.emitAuditEvent(this.audit('enrollment_finalized', enrolled, null, 'enrolled'));
  ...
  async startVerification(input: {
    ...
    this.dependencies.emitAuditEvent(this.audit('verification_issued', enrollment, verificationId, 'issued'));
  ...
  async verifySample(
    ...
    const speakerVerification = await this.dependencies.verifier.verifySpeaker({
      audio: sample.audio,
    ...
    this.dependencies.emitAuditEvent(this.audit('verification_completed', enrollment, sample.verificationId, result.kind));
...
export function defaultVoiceIdServiceConfig(): VoiceIdServiceConfig {
  return {
