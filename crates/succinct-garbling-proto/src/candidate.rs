...
impl CandidateBackendFamily {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::PaillierCompressed => "paillier_compressed",
            Self::LatticeRlwe => "lattice_rlwe",
        }
    }
}
...
fn backend_spec(family: CandidateBackendFamily) -> CandidateBackendSpec {
    ...
    match family {
        ...
        CandidateBackendFamily::PaillierCompressed => {
            let log_n = 3_072u64;
            let public_data_bits = LAMBDA + (3 + 4 * LAMBDA) * 2 * log_n;

            CandidateBackendSpec {
                family,
                source: "Ishai-Li-Lin 2025/442, Table 3 and compressed Paillier formula"
                    .to_string(),
                public_data_bits,
                public_data_bytes: bits_to_bytes(public_data_bits),
                parameter_summary: vec![
                    "lambda=128".to_string(),
                    "Paillier modulus=3072 bits".to_string(),
                    "compressed aHMAC public data".to_string(),
                    "small secret-exponent assumption".to_string(),
                ],
                evaluator_notes: vec![
                    "Public data is larger than the size-optimized prime-order setting but much smaller than lattice.".to_string(),
                    "Arithmetic sits in composite-order groups instead of prime-order groups.".to_string(),
                ],
            }
        }
        CandidateBackendFamily::LatticeRlwe => {
            let n = 8_192u64;
            let log_q = 142u64;
            let public_data_bits = LAMBDA + (3 + 4 * LAMBDA) * n * log_q;

            CandidateBackendSpec {
                family,
                source: "Ishai-Li-Lin 2025/442, Table 3 and lattice formula".to_string(),
                public_data_bits,
                public_data_bytes: bits_to_bytes(public_data_bits),
                parameter_summary: vec![
                    "lambda=128".to_string(),
                    "ring degree n=8192".to_string(),
                    "log_q=142".to_string(),
                    "uniform ternary secrets and rounded Gaussian noise".to_string(),
                ],
                evaluator_notes: vec![
                    "Most accelerator-friendly arithmetic family, but published public data is very large.".to_string(),
                    "Useful as a compute comparison point, not the first size-first backend to pursue.".to_string(),
                ],
            }
        }
    }
