...
/// Signer-side production backend input for Candidate A.
...
/// Signer-side batch input for evaluating every requested Candidate A output.
#[derive(Clone, PartialEq, Eq)]
...
/// Recipient-side production backend input for Candidate A combine.
#[derive(Clone, PartialEq, Eq)]
...
/// Recipient-local combined Candidate A output. Debug output redacts material.
#[derive(Clone, PartialEq, Eq)]
...
/// Evaluates one signer-local Candidate A partial through `threshold-prf`.
    ...
    R: RngCore + CryptoRng,
{
    ...
    let backend_bundle = evaluate_partial_with_dleq_proof(&backend_share, &context, proof_rng)
        .map_err(map_threshold_error)?;
    let binding = crate::derivation::candidate_mpc_prf::MpcPrfPartialBindingV1::from_signer_input(
        &input.signer_input,
...
/// Evaluates every requested signer-local Candidate A output through `threshold-prf`.
...
/// Verifies one Candidate A proof bundle through `threshold-prf`.
...
/// Verifies two Candidate A proof bundles and combines them for the recipient.
pub fn combine_mpc_prf_proof_bundles_with_threshold_backend_v1(
    ...
    let suite_id = input.left.signer_partial.binding.suite_id;
    if suite_id != input.right.signer_partial.binding.suite_id {
        return Err(RouterAbDerivationError::new(
            RouterAbDerivationErrorCode::MalformedInput,
...
fn threshold_context_from_plan_v1(
        ...
        || plan.threshold_prf_suite_label != "threshold-prf/ristretto255-sha512"
    {
        return Err(RouterAbDerivationError::new(
            ...
            "MPC PRF backend requires threshold-prf Ristretto255/SHA-512 suite",
        ));
    }
    if plan.output_encoding != MpcPrfOutputEncodingV1::CanonicalEd25519Scalar32 {
        return Err(RouterAbDerivationError::new(
            RouterAbDerivationErrorCode::MalformedInput,
            "MPC PRF backend requires canonical Ed25519 scalar output",
        ));
    }
