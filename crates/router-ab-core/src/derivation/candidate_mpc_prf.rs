...
/// Candidate A output purpose bound into the underlying threshold PRF context.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// Candidate A output encoding required from the underlying threshold PRF.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum MpcPrfOutputEncodingV1 {
    /// Output must be canonical Ed25519 scalar bytes.
    CanonicalEd25519Scalar32,
}

impl MpcPrfOutputEncodingV1 {
    /// Returns the canonical output-encoding label.
    pub fn as_str(self) -> &'static str {
        match self {
            Self::CanonicalEd25519Scalar32 => "canonical_ed25519_scalar_32",
        }
    }
}

/// Purpose-binding plan for calling `threshold-prf` from Candidate A.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct MpcPrfPurposeBindingPlanV1 {
    /// Candidate A suite.
    pub suite_id: MpcPrfSuiteId,
    /// External `threshold-prf` suite label.
    pub threshold_prf_suite_label: String,
    ...
    pub output_encoding: MpcPrfOutputEncodingV1,
...
impl MpcPrfSignerPartialInputV1 {
    ...
    /// Validates public Candidate A signer input metadata.
    pub fn validate(&self) -> RouterAbDerivationResult<()> {
...
/// Public metadata bound to one Candidate A partial.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Secret Candidate A partial wire bytes. Debug output is always redacted.
...
/// Public Candidate A share-commitment wire bytes.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Candidate A DLEQ proof wire bytes.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// One signer-produced Candidate A partial plus its public binding.
...
/// Candidate A partial after proof verification at the adapter boundary.
#[derive(Clone, PartialEq, Eq)]
...
/// Recipient combiner input for two verified Candidate A partials.
#[derive(Clone, PartialEq, Eq)]
...
/// Boundary input for Candidate A partial proof verification planning.
#[derive(Clone, PartialEq, Eq)]
...
pub fn plan_mpc_prf_purpose_binding_v1(
    ...
    plan_mpc_prf_purpose_binding_for_output_v1(&input.transcript, input.suite_id, request)
}
...
pub(crate) fn plan_mpc_prf_purpose_binding_for_output_v1(
    ...
    let threshold_prf_context_bytes =
        encode_mpc_prf_context_bytes_v1(suite_id, request, output_purpose, transcript_digest)?;
    let threshold_prf_context_digest = mpc_prf_context_digest_v1(&threshold_prf_context_bytes)?;

    Ok(MpcPrfPurposeBindingPlanV1 {
        suite_id,
        threshold_prf_suite_label: suite_id.threshold_prf_suite_label().to_owned(),
        output_purpose,
        threshold_prf_purpose_label: output_purpose.threshold_prf_purpose_label().to_owned(),
        output_encoding: MpcPrfOutputEncodingV1::CanonicalEd25519Scalar32,
        transcript_digest,
...
/// Validates two verified partials before recipient-side Candidate A combine.
pub fn plan_mpc_prf_combine_v1(
    ...
    if left.suite_id != right.suite_id {
        return Err(RouterAbDerivationError::new(
            RouterAbDerivationErrorCode::MalformedInput,
            "MPC PRF combine requires matching suite ids",
        ));
    }
...
fn encode_mpc_prf_context_bytes_v1(
    ...
    push_field(&mut out, suite_id.as_str().as_bytes());
    push_field(&mut out, suite_id.threshold_prf_suite_label().as_bytes());
    push_field(
        &mut out,
