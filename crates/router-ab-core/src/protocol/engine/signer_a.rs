...
/// Platform-agnostic Signer A engine wrapper.
#[derive(Debug, Clone)]
pub struct SignerAEngine<H> {
    host: H,
}

impl<H> SignerAEngine<H> {
    /// Creates a Signer A engine over a host implementation.
    ...
    /// Evaluates Signer A's selected threshold-PRF output batch.
        ...
        R: RngCore + CryptoRng,
    {
        if input.signer_input.signer_role != Role::SignerA {
            return Err(RouterAbDerivationError::new(
                ...
                "Signer A engine requires Signer A batch input",
            ));
        }
