...
use threshold_prf::{
    derive_output_from_signing_root_shares, generate_signing_root, split_signing_root_2_of_3,
    PrfContext, PrfOutputEncoding, PrfPurpose, SigningRootShareWireV1, SuiteId,
};
...
fn share_wire(bytes: [u8; 33]) -> MpcPrfSigningRootShareWireV1 {
...
fn share_wires() -> [MpcPrfSigningRootShareWireV1; 2] {
    ...
    [
        share_wire(SigningRootShareWireV1::from_share(&shares[0]).to_bytes()),
        share_wire(SigningRootShareWireV1::from_share(&shares[2]).to_bytes()),
    ]
}
...
fn threshold_backend_evaluates_verifies_and_combines_client_output() {
    ...
    let root = generate_signing_root(&mut setup_rng);
    let shares = split_signing_root_2_of_3(&root, &mut setup_rng);
    let share_a = share_wire(SigningRootShareWireV1::from_share(&shares[0]).to_bytes());
    let share_b = share_wire(SigningRootShareWireV1::from_share(&shares[2]).to_bytes());
    ...
    let plan = router_ab_core::plan_mpc_prf_purpose_binding_v1(&signer_a, &client_request)
        .expect("purpose plan");
    let direct = derive_output_from_signing_root_shares(
        &[shares[0].clone(), shares[2].clone()],
        &threshold_context_from_router_plan(&plan),
    )
...
fn threshold_backend_separates_client_and_relayer_outputs() {
    ...
    let client_a = evaluate_mpc_prf_signer_partial_with_threshold_backend_v1(
        MpcPrfThresholdSignerInputV1 {
            ...
            signing_root_share_wire: share_wire(
                SigningRootShareWireV1::from_share(&shares[0]).to_bytes(),
            ),
        },
        &mut seeded_rng(12),
    )
    ...
    let client_b = evaluate_mpc_prf_signer_partial_with_threshold_backend_v1(
        MpcPrfThresholdSignerInputV1 {
            ...
            signing_root_share_wire: share_wire(
                SigningRootShareWireV1::from_share(&shares[2]).to_bytes(),
            ),
        },
        &mut seeded_rng(13),
    )
...
fn threshold_backend_rejects_transcript_mismatch() {
    ...
    .expect("bundle");
    let mut mismatched_transcript = signer_a.transcript;
