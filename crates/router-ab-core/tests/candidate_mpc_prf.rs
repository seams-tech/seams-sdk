use router_ab_core::{
    ...
    AccountScope, CandidateId, CorrectnessLevel, DerivationContext, MpcPrfCombinerInputV1,
    MpcPrfDleqProofWireV1, MpcPrfOutputEncodingV1, MpcPrfOutputPurposeV1, MpcPrfOutputRequestV1,
    MpcPrfPartialBindingV1, MpcPrfPartialProofBundleV1, MpcPrfPartialVerificationInputV1,
    MpcPrfPartialWireV1, MpcPrfShareCommitmentWireV1, MpcPrfSignerPartialInputV1,
    MpcPrfSignerPartialV1, MpcPrfSuiteId, MpcPrfVerifiedPartialV1, OpenedShareKind, PublicDigest32,
    RequestKind, Role, RootShareEpoch, RouterAbDerivationErrorCode, SignerSetBinding,
...
fn partial_wire_debug_is_redacted_and_length_checked() {
    let partial =
        MpcPrfPartialWireV1::new(vec![7; MPC_PRF_PARTIAL_WIRE_V1_LEN]).expect("partial wire");
...
fn proof_wire_constructors_check_lengths() {
    let commitment = MpcPrfShareCommitmentWireV1::new(vec![9; MPC_PRF_COMMITMENT_WIRE_V1_LEN])
        .expect("commitment");
...
fn partial_verification_plan_accepts_transcript_bound_bundle() {
    ...
    let plan = plan_mpc_prf_partial_verification_v1(MpcPrfPartialVerificationInputV1 {
        transcript,
        proof_bundle: proof_bundle(Role::SignerA, "role:signer-a:local:sha256-a", 0x0a),
    })
