...
| Correctness hardening | Existing `threshold-prf` DLEQ proof machinery gives a clear path for partial correctness | Minimum Level C only binds delivery; deriver bias needs root-generation, address-verification, or public-share-binding gates | Candidate A has the clearer hardening path |
