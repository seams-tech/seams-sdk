...
This document defines how `router-ab-ecdsa-derivation` is intended to integrate with the
...
- threshold signing key should come from `router-ab-ecdsa-derivation`
...
`router-ab-ecdsa-derivation` must be able to supply:
...
1. `router-ab-ecdsa-derivation` derives role-local additive shares.
...
That means the integrated `router-ab-ecdsa-derivation` path should not preserve:

- threshold signing key derived from `PRF.first`
- separate export key derived from `PRF.second`

Once `router-ab-ecdsa-derivation` is live, export and threshold signing should use the same
logical key.
...
`router-ab-ecdsa-derivation`; it cannot rely on ephemeral presign state.
...
```text
router-ab-ecdsa-derivation-secp256k1-export
```

The retired `router-ab-ecdsa-derivation-secp256k1-key-v1` artifact kind is invalid for active
`router-ab-ecdsa-derivation` export paths.
...
- bootstrap/registration slower than today because Router A/B ECDSA derivation is added
...
- there is no in-place cryptographic migration from the current two-key model
  to `router-ab-ecdsa-derivation`
- existing users must re-register to enter the `router-ab-ecdsa-derivation` one-key model
- the current `prfSecond` export lane is retired when `router-ab-ecdsa-derivation` becomes the
  active EVM scheme
- legacy `prfSecond` export artifacts remain legacy-only and are never accepted
  as `router-ab-ecdsa-derivation` export output
...
- new `router-ab-ecdsa-derivation` accounts are created through re-registration
...
- the product must not present legacy `prfSecond` export as if it were the
  threshold ECDSA key for `router-ab-ecdsa-derivation` accounts
