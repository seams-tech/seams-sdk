...
The goal is not to restate the entire `ecdsa-hss` plan. The goal is to answer
one question:

How should `ecdsa-hss` derive threshold signing shares from a canonical hidden
...
1. `ecdsa-hss` derives one canonical hidden secp256k1 secret `x`.
2. `ecdsa-hss` deterministically derives additive 2-party shares:
   - `x_client`
   - `x_relayer`
   such that:
...
- export: explicit HSS-backed export flow
...
1. `ecdsa-hss` derives one canonical hidden secp256k1 secret `x`.
2. `ecdsa-hss` derives an initial sharing of `x`.
...
- the HSS side can choose the most natural hidden sharing for the canonical
  key
...
- HSS hidden sharing corresponds to canonical `x`
...
- no resharing stage
- no extra setup ceremony beyond HSS and direct share derivation
...
Can `ecdsa-hss` derive additive 2-party shares from canonical hidden `x` in a
...
- canonical derivation:
  ...
  - `d = LE32(m)`
  - `h_x = SHA-512("ecdsa-hss:v1:canonical-x" || context || d)`
  - `x = 1 + (BE512(h_x) mod (n - 1))`
- additive-share derivation:
  - derive `x_client` from
    `SHA-512("ecdsa-hss:v1:additive-share:client" || context || BE32(counter) || BE32(x))`
  - set
