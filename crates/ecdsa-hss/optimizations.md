...
- [benchmarks/ecdsa-hss-wasm/src/runner.mjs](/Users/pta/Dev/rust/simple-threshold-signer/benchmarks/ecdsa-hss-wasm/src/runner.mjs)
...
- change:
  ...
  - ran the first Node-hosted wasm baseline for the `ecdsa-hss` lifecycle
- command:
  - `pnpm benchmark:ecdsa-hss:wasm`
- result:
  - accepted
...
- change:
  - added a profiled wasm sign entrypoint through
    [wasm/eth_signer/src/ecdsa_hss.rs](/Users/pta/Dev/rust/simple-threshold-signer/wasm/eth_signer/src/ecdsa_hss.rs)
...
- commands:
  - `cargo bench --manifest-path crates/ecdsa-hss/Cargo.toml --bench performance_baseline`
  - `cargo bench --manifest-path crates/ecdsa-hss/Cargo.toml --bench performance_baseline -- presign_after_start`
- result:
  - accepted
- split medians:
...
- change:
  ...
  - added an allocation-light internal advance path for the 2-party presign
    loop, then rewired `ecdsa-hss` to use it
...
- change:
  - added a non-allocating internal presign event path in
    [crates/signer-core/src/threshold_ecdsa.rs](/Users/pta/Dev/rust/simple-threshold-signer/crates/signer-core/src/threshold_ecdsa.rs)
  - switched the `ecdsa-hss` sign driver to use that internal path plus
    `is_triples_done()` instead of hot-loop string comparisons in
    [crates/ecdsa-hss/src/integration/mod.rs](/Users/pta/Dev/rust/simple-threshold-signer/crates/ecdsa-hss/src/integration/mod.rs)
- commands:
  - `cargo test --manifest-path crates/ecdsa-hss/Cargo.toml`
  - `cargo bench --manifest-path crates/ecdsa-hss/Cargo.toml --bench performance_baseline -- 'sign_bridge_full|presign_protocol_roundtrip|presign_before_start'`
  - `pnpm benchmark:ecdsa-hss:wasm`
