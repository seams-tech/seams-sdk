...
use super::context::{encode_context, EcdsaHssStableKeyContext};
use crate::wire::ServerEvalOperation;

const CONTEXT_BINDING_DOMAIN: &[u8] = b"ecdsa-hss:role-local:v2:context-binding";
const CLIENT_SHARE_DOMAIN: &[u8] = b"ecdsa-hss:role-local:v2:client-share";
const RELAYER_SHARE_DOMAIN: &[u8] = b"ecdsa-hss:role-local:v2:relayer-share";
const PUBLIC_TRANSCRIPT_DOMAIN: &[u8] = b"ecdsa-hss:role-local:v2:public-transcript";
...
pub fn context_binding(context: &EcdsaHssStableKeyContext) -> EcdsaHssResult<[u8; 32]> {
...
pub fn derive_client_share(
    ...
    y_client32_le: [u8; 32],
) -> EcdsaHssResult<ClientRoleShare> {
    let context_bytes = encode_context(context)?;
...
) -> EcdsaHssResult<RelayerRoleShare> {
...
) -> EcdsaHssResult<(RelayerRoleShare, PublicIdentity)> {
        ...
        let identity = match compose_public_identity(
            ...
            Err(err) if is_identity_public_key_sum_error(&err) => {
                relayer_share_retry_counter = relayer_share_retry_counter
                    .checked_add(1)
                    .ok_or_else(|| EcdsaHssError::internal("relayer retry counter overflow"))?;
                continue;
            }
...
pub fn reconstruct_export_key(
    ...
    let x_export =
        Option::<NonZeroScalar>::from(NonZeroScalar::new(*x_client.as_ref() + *x_relayer.as_ref()))
            .ok_or_else(|| EcdsaHssError::invalid_input("export key reconstructed to zero"))?;
...
fn context_binding_from_bytes(context_bytes: &[u8]) -> EcdsaHssResult<[u8; 32]> {
    let frame = frame_digest_input(
...
fn frame_digest_input(domain: &[u8], fields: &[(u8, &[u8])]) -> EcdsaHssResult<Vec<u8>> {
    ...
    for (tag, value) in fields {
        let len = u16::try_from(value.len())
            .map_err(|_| EcdsaHssError::invalid_length("digest frame field exceeds u16 length"))?;
        out.push(*tag);
...
fn parse_nonzero_scalar_32_be(bytes: &[u8], field_name: &str) -> EcdsaHssResult<NonZeroScalar> {
    if bytes.len() != 32 {
...
fn validate_public_key33(public_key33: &[u8; 33], field_name: &str) -> EcdsaHssResult<[u8; 33]> {
...
fn public_key_sum_is_identity(left33: &[u8; 33], right33: &[u8; 33]) -> EcdsaHssResult<bool> {
    let left = PublicKey::from_sec1_bytes(left33)
        .map_err(|_| EcdsaHssError::decode_error("left public key is invalid"))?;
    let right = PublicKey::from_sec1_bytes(right33)
        .map_err(|_| EcdsaHssError::decode_error("right public key is invalid"))?;
    let sum = ProjectivePoint::from(*left.as_affine()) + ProjectivePoint::from(*right.as_affine());
    Ok(bool::from(sum.is_identity()))
}

fn is_identity_public_key_sum_error(err: &EcdsaHssError) -> bool {
    err.message.contains("identity point")
}

fn vec_to_fixed_20(bytes: Vec<u8>, field_name: &str) -> EcdsaHssResult<[u8; 20]> {
    if bytes.len() != 20 {
        return Err(EcdsaHssError::invalid_length(format!(
    ...
    bytes.try_into().map_err(|_| {
        EcdsaHssError::invalid_length(format!("{field_name} must be exactly 20 bytes"))
    })
}

fn vec_to_fixed_32(bytes: Vec<u8>, field_name: &str) -> EcdsaHssResult<[u8; 32]> {
    if bytes.len() != 32 {
        return Err(EcdsaHssError::invalid_length(format!(
            "{field_name} must be 32 bytes (got {})",
            bytes.len()
    ...
    bytes.try_into().map_err(|_| {
        EcdsaHssError::invalid_length(format!("{field_name} must be exactly 32 bytes"))
    })
}

fn vec_to_fixed_33(bytes: Vec<u8>, field_name: &str) -> EcdsaHssResult<[u8; 33]> {
    if bytes.len() != 33 {
        return Err(EcdsaHssError::invalid_length(format!(
    ...
    bytes.try_into().map_err(|_| {
        EcdsaHssError::invalid_length(format!("{field_name} must be exactly 33 bytes"))
    })
}
