...
use crate::ddh::ddh_hss::{
    prepare_client_ot_sender_state_words_public, role_views_for_backend,
    DdhHssPreparedOtSenderStateWord,
};
...
use crate::ddh::{
    ...
    DdhHiddenEvalRun, DdhHssBackend, DdhHssEvaluationKey, DdhHssEvaluator, DdhHssGarbler,
    DdhHssInputShareBundle, DdhHssOtInputBundleOffer, DdhHssOtReceiverStateBundle,
    DdhHssOtReconstructTiming, DdhHssOtReleasedRemoteBundle, DdhHssOtRemoteBundle,
    DdhHssOtResponseBundle, DdhHssOtSelectionBundle, DdhHssOtSenderStateBundle, DdhHssShareSide,
...
pub struct PrimeOrderSuccinctHssGarblerSession {
    ...
    client_ot_offer: PrimeOrderSuccinctHssClientOtOffer,
    garbler_ot_state: PrimeOrderSuccinctHssGarblerOtState,
    y_client_sender_words_prepared: Vec<DdhHssPreparedOtSenderStateWord>,
    tau_client_sender_words_prepared: Vec<DdhHssPreparedOtSenderStateWord>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct PrimeOrderSuccinctHssGarblerPreparedOtState {
    y_client_sender_words_prepared: Vec<DdhHssPreparedOtSenderStateWord>,
    tau_client_sender_words_prepared: Vec<DdhHssPreparedOtSenderStateWord>,
}

fn prepare_garbler_ot_state_for_session(
    client_ot_offer: &PrimeOrderSuccinctHssClientOtOffer,
    garbler_ot_state: &PrimeOrderSuccinctHssGarblerOtState,
) -> ProtoResult<PrimeOrderSuccinctHssGarblerPreparedOtState> {
    Ok(PrimeOrderSuccinctHssGarblerPreparedOtState {
...
impl PrimeOrderSuccinctHssPreparedSession {
    ...
    pub fn evaluate_hidden_run_with_timing(
        &self,
        input: &FExpandInput,
    ) -> ProtoResult<(DdhHiddenEvalRun, PrimeOrderSuccinctHssEvaluateTiming)> {
        self.ensure_input_context(input)?;
        let runtime = &self.shared_runtime_cached;
        let garbler_session = &self.garbler_session_cached;
        let evaluator_session = &self.evaluator_session_cached;
...
impl PrimeOrderSuccinctHssGarblerSessionState {
    pub fn materialize(&self) -> ProtoResult<PrimeOrderSuccinctHssGarblerSession> {
        let prepared_ot_state =
            prepare_garbler_ot_state_for_session(&self.client_ot_offer, &self.garbler_ot_state)?;
        Ok(PrimeOrderSuccinctHssGarblerSession {
