...
- Public progress events are emitted by SeamsWeb flows as v2 `WalletFlowEvent` payloads:
  - `client/src/SeamsWeb/near/actions.ts`
...
File: `client/src/SeamsWeb/walletIframe/client/router.ts`
...
See: `client/src/SeamsWeb/walletIframe/client/overlay/overlay-controller.ts` for the single source of truth that applies these CSS mutations and manages sticky mode.
...
- File: `client/src/SeamsWeb/walletIframe/client/transport/IframeTransport.ts`
...
- Pre‑warm to reduce perceived latency before the overlay appears:
  ...
  - Sources: `client/src/SeamsWeb/index.ts` and `client/src/core/signingEngine/nonce/NonceCoordinator.ts`.
