import { parseLinkedDeviceSessionRecordV1, type LinkedDeviceSessionRecordV1 } from '../../../../core/deviceLinking/linkedDeviceSession';
...
export function parseD1LinkedDeviceSessionRowV1(
  ...
  if (
    record.linkSessionId !== linkSessionId ||
  ...
  const expectedClaimExpiry = record.state.state === 'claimed_by_owner' ? record.state.claimExpiresAtMs : record.claimTranscript?.value.claimExpiresAtMs;
  if ((claimExpiresAtMs ?? null) !== (expectedClaimExpiry ?? null)) {
...
export function parseD1LinkedDeviceSessionTranscriptRowV1(
...
): { readonly kind: 'claim' | 'approval'; readonly digestB64u: DigestB64u; readonly transcriptJson: unknown; readonly createdAtMs: number } {
  ...
  const digestB64u = requiredDigest(row.digest_b64u, 'digest_b64u');
  const transcriptJson = parseJson(row.transcript_json, 'transcript_json');
  const createdAtMs = requiredPositiveInteger(row.created_at_ms, 'created_at_ms');
  return { kind, digestB64u, transcriptJson, createdAtMs };
}
