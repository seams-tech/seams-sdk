...
- Added `tests/wallet-iframe/static-wallet-assets.browser.test.ts`, which serves `packages/wallet/dist/public`, loads the generated `/wallet-service` page, constructs every wallet worker as a module worker, and fetches/compiles every worker WASM companion from the static wallet origin.
...
- `pnpm -C packages/wallet run check:static-wallet-assets`
- `pnpm -C packages/wallet exec tsc -p tsconfig.build.json --noEmit`
- `pnpm -C packages/wallet run build:sdk`
...
- Added build-owned hosted wallet asset emission for `packages/wallet/dist/public`, including `/sdk/*`, `/sdk/workers/*`, `/wallet-service`, `/export-viewer`, `wallet-assets.manifest.json`, and `headers.manifest.json`.
...
- Changed repo-local Caddy so `https://localhost:8443` serves `packages/wallet/dist/public` directly and app-origin `/sdk/*`, `/wallet-service`, and `/export-viewer` return 404.
- Removed `@seams/wallet/plugins/vite` from the site Vite config and related app-side env/type/path plumbing.
...
- Recorded `packages/wallet/scripts/checks/assert-hosted-wallet-docs.mjs` in the Refactor 89 source-guard ledger.

Validation:

- `pnpm --dir packages/wallet build:sdk`
- `pnpm --dir packages/wallet build:prod`
- `pnpm --dir packages/wallet check:static-wallet-assets`
- `pnpm --dir packages/wallet smoke:static-wallet-origin`
- `pnpm --dir packages/wallet build:check:fresh`
- `pnpm --dir apps/seams-site typecheck`
- `pnpm --dir apps/seams-site exec vite build`
- `SEAMS_WALLET_PUBLIC_ROOT=/Users/pta/Dev/rust/seams-sdk/packages/wallet/dist/public caddy validate --config apps/seams-site/Caddyfile --adapter caddyfile`
- `pnpm --dir packages/wallet exec tsc -p tsconfig.build.json --noEmit`
- `pnpm --dir packages/wallet check:hosted-wallet-docs`
- `pnpm --dir packages/wallet build:check:fresh`
