...
2. Renaming wallet "login/logout" to wallet "unlock/lock".
...
2. Wallet state is `locked`/`unlocked` (never "logged in/out").
...
1. Delete old route handlers and symbols that express wallet login semantics.
2. Delete old client API names that refer to wallet login/logout.
...
2. Provider-specific glue only where needed (claims mapping, logout/revocation hooks).
...
1. Publish Better Auth integration guide and sample middleware.
2. Publish Auth0 guide (OIDC + optional SAML enterprise notes).
3. Publish Okta guide (OIDC + inbound federation mapping notes).
4. Publish Google SSO guide (GIS -> backend token verify -> session exchange).
5. Publish optional quickstarts for Clerk/Supabase/Firebase using same OIDC/JWT adapter.
...
6. No legacy login-named routes/symbols remain after refactor lands.
