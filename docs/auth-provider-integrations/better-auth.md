...
Use Better Auth for app login while delegating wallet warm-session transport to Router API `session/*` routes.

This guide keeps app auth and wallet state decoupled:

1. Better Auth proves user identity.
2. Your backend exchanges that proof to Router API `POST /session/exchange`.
3. Wallet state transitions remain in Router API `wallet/*`.

## Prerequisites

1. Better Auth is already configured in your app/backend.
2. Router API exposes:
- `POST /session/exchange`
- `POST /session/refresh`
- `POST /session/revoke`
- `GET /session/state`
3. Router API OIDC/JWT verifier config trusts your Better Auth issuer/audience.

## Integration Flow

1. User signs in using Better Auth.
2. Backend verifies Better Auth session/token and resolves a stable subject (`sub`).
3. Backend sends the verified token to Router API:
- `exchange.type=oidc_jwt`
- `exchange.token=<verified id token or backend-issued exchange JWT>`
4. Router API returns:
- JWT session payload (`sessionKind=jwt`) or
- HttpOnly cookie (`sessionKind=cookie`)
5. Client continues wallet operations via Router API using `session/*` and `wallet/*`.
...
2. `iss` must match Router API allowlist.
3. `aud` must match Router API allowlist.
4. Optional org/tenant claims can be forwarded for scoped webhook emission (`orgId`, `org_id`, `tenantId`, `tenant_id`).

## Session Lifecycle Mapping

1. Better Auth sign-out in your app should call Router API `POST /session/revoke` and `POST /wallet/lock`.
2. Wallet lock action should call Router API `POST /wallet/lock`.
3. Use Router API `POST /session/refresh` for sliding warm-session renewal.
4. Observe lifecycle events through Router API webhooks:
