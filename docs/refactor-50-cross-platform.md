# Refactor 50: Cross-Platform SDK Readiness

Date created: 2026-05-27
Status: closeout. Phases 0, 1, 1.5, the ECDSA signer-crypto boundary slice,
Phase 2 typed durable ECDSA repository wiring, Phase 3 ECDSA persistence/key-ref
cleanup, and the Phase 5 exact ECDSA/Ed25519 provisioning input slice are
complete. Phase 6B corrected ECDSA HSS crate ownership, and the Rust
signer-core command plus browser WASM prepare/finalize adapter have landed.
Refactor 50 now closes as historical implementation context. Remaining
production cutover, compatibility deletion, export/recovery, activation/restore,
schema generation, and native-readiness work move to
`docs/refactor-51-cross-platform-2.md`. Original native API exploration is
removed from Refactor 50.
...
- TypeScript should call coarse signer commands instead of composing crypto
  helper steps.
- Rust protocol/core crates should own derivation labels, salts, participant
  validation, threshold parameters, share mapping, internal protocol
  serialization, and lifecycle-sensitive crypto state.
...
4. Move passkey PRF to client-root derivation behind the secret-source boundary,
   preferably into the signer-core command for the ECDSA HSS slice so
   TypeScript no longer owns the HKDF label.
...
6. Prefer coarse signer-core commands over exposing crypto helper
...
MVP signer-crypto ownership:

- validate `ClientSecretSource` branch inputs after TypeScript boundary parsing
- keep ECDSA HSS protocol labels, derivation, share mapping, secp256k1 public
  identity, and transcript framing in `crates/ecdsa-hss`
- derive command-level client root share material inside `crates/signer-core`
  from accepted secret-source branches
- construct pending and ready role-local client state as opaque signer-core
  state blobs
- return public facts needed by TypeScript routing and persistence
- map protocol and command failures to typed signer-crypto error codes
...
- `docs/refactor-50-cross-platform-inventory.md`, or a section appended to this
...
those operations. Phase 6 implements the signer-core command behind the same
operations and exposes it through `wasm/hss_client_signer`. Keeping those steps
separate lets the SDK call sites move to a stable transport-free contract before
the browser WASM binding starts calling the signer-core command. ECDSA HSS
protocol primitives remain owned by `crates/ecdsa-hss`.
...
- [x] Keep direct-call and native-call adapters possible for future iOS/Linux.
- [x] Convert boolean-success worker results to `Result`-style unions where the
...
- The threshold ECDSA presign abort worker result uses a concrete
  `threshold_ecdsa_presign_session_aborted` lifecycle branch instead of a
  boolean success bag.
...
- [x] Define `email_otp_worker_session` as a worker-owned opaque handle for
      Email OTP ECDSA session bootstrap.
- [x] Delete the unused EVM-family Email OTP provision-plan branch that still
      carried root-share bytes.
- [x] Remove the remaining active Email OTP root-share paths from
      registration-prep material and export/recovery flows.
- [x] Make ECDSA and Ed25519 provisioning functions accept exact supported
      secret-source branches.
  - [x] Change the ECDSA client-root resolver from an optional argument bag to
        exact request branches for WebAuthn PRF credentials, collected WebAuthn
        PRF credentials, and transitional raw client-root shares.
  - [x] Move EVM-family ECDSA step-up provision plans from top-level
        `clientRootShare32B64u`, `webauthnAuthentication`, and
        `emailOtpAuthContext` fields to exact `provisionSecretSource` branches.
        Activation builders still unwrap transitional root shares until Phase 6.
  - [x] Move Ed25519 session mint and reconnect authorization from raw
        `localPrfCredential`, `webauthnAuthentication`, and
        `localPrfFirstB64u` fields to exact `localSecretSource` and
        `policySecretSource` branches.
...
## Phase 6: Add The Signer-Core Command And Browser WASM Adapter

Use the existing browser Rust/WASM crypto baseline to move one helper-shaped
TypeScript workflow behind a coarse signer-core command. This phase creates the
cross-platform command surface and the browser WASM binding for it. It does not
move ECDSA HSS protocol ownership out of `crates/ecdsa-hss`.
...
- `crates/ecdsa-hss` owns ECDSA HSS protocol behavior: derivation labels,
  role-local derivation, additive share mapping, secp256k1 public identity,
  transcript framing, export reconstruction, and protocol errors.
- `crates/signer-core` owns the cross-platform prepare/finalize command:
  accepted secret-source handling, pending/ready state construction, internal
  serialization, public fact validation, and command error mapping.
- `wasm/hss_client_signer` owns only the browser WASM binding for the
  signer-core command.

Implementation home:

- Reusable ECDSA HSS protocol primitives stay in `crates/ecdsa-hss`: stable
  context encoding, role-local derivation, share mapping, public identity
  composition, transcript digests, export reconstruction, and protocol wire
  enums.
- `crates/signer-core` owns cross-platform command envelopes, accepted
  secret-source normalization, pending state serialization, ready state
  finalization, public fact validation, and error mapping. It imports
  `ecdsa-hss` for protocol primitives.
- `wasm/hss_client_signer` exposes the browser WASM binding for the signer-core
  command.
- Native bindings later wrap the same signer-core command.

Implementation dependency order:

- [x] Complete Phase 6B dependency correction before exposing the coarse command
      through browser WASM or deleting TypeScript assembly paths.
- [x] Add the coarse prepare/finalize command on top of the corrected
      signer-core command boundary.
- [x] Expose the coarse command through `wasm/hss_client_signer`.
...
- [x] Implement the existing `prepareEcdsaClientBootstrap` and
      ...
      is unsound.
- [x] Add the reusable signer-core command that accepts normalized secret
      sources, calls `ecdsa-hss` for ECDSA HSS protocol derivation/share
      behavior, validates public facts, serializes pending state, and finalizes
      ready state.
  - [x] Import reusable stable-context, role-local derivation, share mapping,
        public identity, transcript digest, and export reconstruction primitives
        from `ecdsa-hss`.
  - [x] Add signer-core pending/ready blob serialization and finalize-time
        relayer public identity validation.
- [x] Expose the signer-core command through `wasm/hss_client_signer`.
- [x] Return `{ pendingStateBlob, clientBootstrap, publicFacts }` from prepare
      and `{ stateBlob, publicFacts }` from finalize.
- [x] Hand off production TypeScript helper-pipeline replacement to Refactor 51
      Phase 6.
- [x] Hand off deletion of the replaced TypeScript crypto-internal assembly path
...
Refactor 50 slice acceptance:

- The signer-core prepare/finalize command and browser WASM binding exist for
  the current `SignerCryptoPort` contract.
- The new browser WASM prepare/finalize exports call `crates/signer-core`
  command functions; signer-core gets ECDSA HSS protocol behavior from
  `crates/ecdsa-hss`.
...
- Future native SDKs can call the same Rust core command with a platform-built
  secret source.
- The old TypeScript assembly path removal is tracked in Refactor 51 Phase 6 and
  Phase 9.
...
## Phase 6B: Correct ECDSA HSS Crate Ownership

Phase 6B corrects the dependency direction discovered during Phase 6 review.
`ecdsa-hss` remains an independent protocol library. `signer-core` depends on
that protocol library to build cross-platform signer commands.

### Phase 6B Objective

Restore the crate layering before the browser WASM coarse command becomes the
active production path. The corrected layering keeps protocol behavior in the
protocol crate and puts cross-platform command orchestration in signer-core.
...
```text
crates/ecdsa-hss -> crates/signer-core
```

Allowed lower-level dependencies:

- `crates/ecdsa-hss` may depend directly on `k256`, `sha2`, `zeroize`, and small
  encoding helpers needed by the ECDSA HSS protocol.
- If secp256k1 helper sharing becomes too broad, extract a small lower-level
  primitive crate such as `crates/signer-crypto-primitives`. Both `ecdsa-hss`
  and `signer-core` may depend on that crate.
- Phase 6B should prefer restoring direct `ecdsa-hss` protocol dependencies over
  introducing a new crate unless duplication becomes material.

### Module Layout After Phase 6B

`crates/ecdsa-hss` owns these protocol modules:

```text
crates/ecdsa-hss/src/shared/context.rs
crates/ecdsa-hss/src/shared/derive.rs
crates/ecdsa-hss/src/wire/mod.rs
```

Those files contain owned implementations rather than `pub use
signer_core::...` re-exports.

`crates/signer-core` owns command modules only:

```text
crates/signer-core/src/threshold_ecdsa_hss/command.rs
crates/signer-core/src/threshold_ecdsa_hss/mod.rs
```

The signer-core module may re-export command types and command functions:
...
- `prepare_ecdsa_client_bootstrap(...)`
- `finalize_ecdsa_client_bootstrap(...)`

The signer-core module should not re-export `ecdsa-hss` protocol primitives as
if signer-core owned them. Code that needs protocol primitives imports them from
`ecdsa_hss`.

### `ecdsa-hss` Protocol Ownership

`ecdsa-hss` owns:

- `EcdsaHssStableKeyContext`
- context encoding, domain tags, scheme id, curve id, key scope, and participant
  ids
- role-local client share derivation
- role-local relayer share derivation
- retry-counter behavior for non-identity public key sums
- additive share mapping into threshold-signatures participant shares
- `ClientRoleShare`
- `RelayerRoleShare`
- `PublicIdentity`
- public identity composition from role shares
- public identity composition from public keys
- transcript digest framing
- `ServerEvalOperation`
- `AllowedOutputKind`
- export key reconstruction
- protocol fixtures and vector tests
- protocol benchmarks, specs, and formal-verification entry points

`ecdsa-hss` error handling should use its own protocol-facing error type or a
small local alias. It should not import `SignerCoreError`.

### `signer-core` Command Ownership

`signer-core` owns:

- command request and response structs for prepare/finalize
- accepted `ClientSecretSource` branches after TypeScript/platform boundary
  normalization
- decoding WebAuthn PRF, Email OTP worker-session, and future native secret
  source inputs into the fixed protocol inputs required by `ecdsa-hss`
- opaque pending blob serialization
- opaque ready blob serialization
- public facts returned to TypeScript/WASM/native callers
- command-level validation that compares relayer public identity supplied by the
  relayer with protocol public identity computed through `ecdsa-hss`
- command-level error mapping into signer-core error codes

`signer-core` should call `ecdsa_hss::shared::context`,
`ecdsa_hss::shared::derive`, and `ecdsa_hss::wire` for ECDSA HSS protocol
behavior.

### Cargo Ownership

`crates/ecdsa-hss/Cargo.toml` should contain only protocol dependencies.
Expected direct dependencies for the restored protocol implementation:

- `k256`
- `sha2`
- `zeroize`
- fixture/test-only dependencies as needed

`crates/ecdsa-hss/Cargo.toml` must not contain:

- `signer-core`

`crates/signer-core/Cargo.toml` should depend on:

- `ecdsa-hss = { path = "../ecdsa-hss" }` behind the feature set used by the
  ECDSA prepare/finalize command

`wasm/hss_client_signer/Cargo.toml` continues to depend on `signer-core` for the
browser export surface.

### Migration Steps

1. Restore protocol modules in `crates/ecdsa-hss`.
   - Move the owned implementation for `context`, `derive`, and `wire` back
     into `ecdsa-hss`.
   - Restore the direct protocol dependencies removed from
     `crates/ecdsa-hss/Cargo.toml`.
   - Keep existing `ecdsa-hss` public module paths stable.

2. Narrow signer-core exports.
   - Remove `pub use` exports for protocol primitives from
     `crates/signer-core/src/threshold_ecdsa_hss/mod.rs`.
   - Keep only command types, blob types, public facts, and prepare/finalize
     command functions in signer-core exports.

3. Update signer-core command internals.
   - Import protocol types and functions from `ecdsa_hss`.
   - Map `ecdsa-hss` protocol errors into `SignerCoreError` at the command
     boundary.
   - Keep pending/ready blob serialization in signer-core.

4. Verify the dependency direction during implementation review.
   - Confirm `crates/ecdsa-hss/Cargo.toml` has no `signer-core` dependency.
   - Confirm `crates/signer-core/Cargo.toml` depends on `ecdsa-hss` behind the
     ECDSA HSS command feature.
   - Confirm signer-core public exports expose only command-owned types and
     functions.

5. Re-run parity and protocol tests.
   - First run Rust protocol tests.
   - Then run signer-core command tests.
   - Then run WASM/browser parity tests after the dependency graph is corrected.

### Review Checks

Check the corrected ownership directly while reviewing the Phase 6B diff:

- `crates/ecdsa-hss/Cargo.toml` must not include `signer-core`.
- `crates/ecdsa-hss/src/**` must not import `signer_core::`.
- `crates/signer-core/src/threshold_ecdsa_hss/mod.rs` must not re-export
  `context`, `derive`, or `wire` protocol modules.
- New prepare/finalize exports in `wasm/hss_client_signer` must call signer-core
  command APIs. Existing direct `ecdsa-hss` protocol calls in legacy raw
  bootstrap/export functions are deleted with the raw-share assembly path.

### Compatibility And Deletion Policy

- Do not keep duplicate live implementations of ECDSA HSS protocol derivation.
  During the move, duplicate code may exist only inside the active edit and must
  be removed before Phase 6B is marked complete.
- Do not keep compatibility re-exports from `ecdsa-hss` to signer-core after
  the ownership correction.
- Keep TypeScript migration compatibility at persistence/request boundaries
  only, then delete raw-share paths when the coarse command is active.

### Acceptance Criteria

- `crates/ecdsa-hss` builds and tests as an independent ECDSA HSS protocol
  crate.
- `crates/ecdsa-hss` has no `signer-core` dependency.
- `crates/signer-core` depends on `ecdsa-hss` for ECDSA HSS protocol behavior.
- signer-core exposes only command, blob, public-fact, and error-mapping
  surfaces for the ECDSA HSS prepare/finalize path.
- browser WASM exports the signer-core prepare/finalize command after the crate
  graph is corrected.
- the new signer-core prepare/finalize command hides role-local derivation
  labels, share mapping, secp256k1 validation, and command-owned blob layout
  from its TypeScript callers.
...
- `pnpm -C tests exec playwright test ./unit/browserPlatformRuntime.signerCrypto.unit.test.ts --reporter=line`

### Phase 6B Completion

- [x] Restored owned protocol modules in `crates/ecdsa-hss`.
- [x] Removed the `ecdsa-hss` dependency on `signer-core`.
- [x] Added the `signer-core` dependency on `ecdsa-hss` behind the ECDSA HSS
      command feature.
- [x] Kept signer-core exports limited to command, blob, public-fact, and
      error-mapping surfaces.
- [x] Exposed the new browser WASM prepare/finalize command functions through
      `wasm/hss_client_signer`.
- [x] Ran protocol, signer-core, WASM, browser parity, guard, formatting, and
      formal-verification checks after the ownership correction.

## Phase 7/8 Closeout: High-Impact Successor Work Only

The original Phase 7 and Phase 8 were exploratory. Refactor 50 keeps only the
successor work that protects current browser UX and removes active raw-material
paths.

Refactor 51 owns the successor implementation:

- Phase 7: move export/recovery onto ready blobs, keep export all-or-nothing,
  require fresh export-scoped authorization, and prevent transaction warm-session
  or step-up authority from authorizing export.
- Phase 8: move signing-session activation and refresh restore onto exact typed
  branches, preserve same-method auth separation, restore only exact valid warm
  lanes, and ensure restored transaction sessions never become export authority.

Closed tasks:

- [x] Close Refactor 50 Phase 7/8 as implementation work.
- [x] Keep only the Refactor 51 Phase 7/8 behavior-preservation successor tasks
      as actionable follow-up.

Validation:

- Documentation review only. Production export, activation, and restore work
  belongs in Refactor 51 implementation packets.
...
8. Phase 6: signer-core command and browser WASM binding for ECDSA bootstrap.
   Implement command orchestration, validation, pending state serialization, and
   ready state finalization in `crates/signer-core`; keep ECDSA HSS protocol
   primitives in `crates/ecdsa-hss`; expose the command through
   `wasm/hss_client_signer`.
9. Phase 7/8: closed here; high-impact successor work is tracked in Refactor 51
   Phase 7 and Phase 8.
...
```ts
export function parseRawEcdsaRoleLocalRecord(raw: unknown): EcdsaRoleLocalRecordParseResult;
```
...
### Phase 6 Signer-Core Command And Browser WASM Spec

Rust implementation targets:

- `crates/ecdsa-hss`: reusable protocol implementation for stable context
  encoding, role-local derivation, additive-share mapping, public identity,
  transcript digests, export reconstruction, and protocol wire enums.
- `crates/signer-core`: reusable command implementation, accepted
  secret-source handling, pending/ready state serialization, public fact
  validation, and error codes. The command calls `ecdsa-hss` for ECDSA HSS
  protocol behavior.
...
3. Secret-source guard:
   ...
   - allowlist browser adapter internals and signer-core/browser WASM binding
     tests
...
| 6 signer-core command + browser WASM binding | `cargo test --manifest-path crates/signer-core/Cargo.toml`; `cargo test --manifest-path wasm/hss_client_signer/Cargo.toml`; parity fixture tests; targeted browser HSS tests.             |
| 6B crate ownership                           | `cargo test --manifest-path crates/ecdsa-hss/Cargo.toml`; `cargo test --manifest-path crates/signer-core/Cargo.toml`; source review confirms `ecdsa-hss` has no `signer-core` dependency. |
| 7/8 closeout                                 | Documentation review only; high-impact successor work is tracked in Refactor 51 Phase 7 and Phase 8.                                                                                      |
...
- [x] Update `SignerWorkerManager` so storage, authenticator, and worker
      transport dependencies are injected through runtime construction.

### ECDSA Secret Source Boundary

- [x] Hand off final canonical ECDSA client-secret source boundary promotion to
      Refactor 51 Phase 3 and Phase 5.
  - [x] Route WebAuthn credential branches in
        `resolveThresholdEcdsaClientRootShare(...)` through
        `buildWebAuthnPrfFirstSecretSource(...)`; raw client-root share
        branches remain explicit and tracked for Phase 6 removal.
...
- [x] Add type fixtures rejecting broad spreads and raw-record casts into
      `ClientSecretSource`.
- [x] Make `email_otp_worker_session` a worker-owned opaque handle and remove
      active core paths that pass Email OTP root-share bytes through TypeScript.
  - [x] Move Email OTP ECDSA login and registration session bootstrap from
        `clientRootShare32B64u` worker payloads to one-shot worker-issued
        handles.
  - [x] Delete the unused EVM-family Email OTP provision-plan builder that
        accepted `clientRootShare32B64u`.
  - [x] Move Email OTP wallet-registration ECDSA prepared-client bootstrap from
        `clientRootShare32B64u` registration material to a
        `wallet_registration_ecdsa_prepare` worker-issued handle.
  - [x] Remove remaining Email OTP root-share bytes from registration-prep
        material and export/recovery paths.
    - [x] Registration-prep material.
    - [x] Export/recovery paths.
          Email OTP Ed25519 export now invokes
          `exportEmailOtpEd25519SeedWithAuthorization` and receives only the final
          seed export artifact; the recovered PRF material stays inside the Email
          OTP worker.
- [x] Keep iOS and embedded branches as typed unsupported dispatch failures in
      the browser adapter.
- [x] Hand off passkey PRF client-root HKDF cutover behind
      `SignerCryptoPort.prepareEcdsaClientBootstrap` to Refactor 51 Phase 6.
- [x] Decision: reusable ECDSA HSS protocol primitives belong in
      `crates/ecdsa-hss`; the cross-platform prepare/finalize command belongs
      in `crates/signer-core` and is exposed to the browser through
      `wasm/hss_client_signer`.
...
- [x] Implement the `hssClient` worker request/result contract for coarse
      prepare/finalize commands.
- [x] Add the reusable signer-core command for accepted secret-source handling,
      public fact validation, pending state serialization, and ready state
      finalization. The command uses `ecdsa-hss` for role-local derivation,
      additive-share mapping, transcript digest, and public identity logic.
- [x] Update `wasm/hss_client_signer` to call signer-core and return public
      facts plus pending and ready blob envelopes.
- [x] Hand off active-bootstrap removal of `clientShare32B64u`,
      `mappedPrivateShare32B64u`, and `verifyingShare33B64u` to Refactor 51
      Phase 6 and Phase 9.
- [x] Add `prepareEcdsaClientBootstrap(...)` and
      `finalizeEcdsaClientBootstrap(...)` wrappers over the signer-core-backed
      browser WASM exports. Production callsite cutover remains in Refactor 51
      Phase 6.
- [x] Hand off wallet registration bootstrap and threshold ECDSA session
      bootstrap production cutover to Refactor 51 Phase 6.
- [x] Hand off Email OTP worker ECDSA bootstrap cutover to the same coarse
      signer-core command shape to Refactor 51 Phase 6.

### Persistence And Key Refs

- [x] Hand off replacement of `ThresholdEcdsaHssRoleLocalClientState` raw share
      fields with `EcdsaRoleLocalReadyStateBlob` and required public identity
      fields to Refactor 51 Phase 3, Phase 4, and Phase 6.
...
- [x] Hand off final `ThresholdEcdsaBackendBinding` opaque-state or typed-handle
      cutover to Refactor 51 Phase 3, Phase 4, Phase 6, and Phase 8.
...
- [x] Hand off `clientAdditiveShare32B64u` removal from active core signing
      paths to Refactor 51 Phase 6 and Phase 9 after the opaque state path is
      complete.

### Export And Recovery

- [x] Update ECDSA HSS export to consume the role-local boundary parser.
- [x] Stop re-deriving export material from `clientRootShare32B64u` in the
      active export path.
  - [x] Passkey ECDSA export now passes the verified WebAuthn credential to the
        export boundary, which resolves client-root material through the
        existing client-secret source resolver instead of transporting
        `clientRootShare32B64u` through the export flow.
- [x] Update passkey recovery export flow to collect a `ClientSecretSource`
      only when the platform adapter needs to unlock or refresh state.
  - [x] Cached canonical ECDSA export artifacts are displayed through the secure
        viewer without collecting a WebAuthn PRF credential, while uncached
        passkey exports still collect the credential needed by the export
        boundary.
- [x] Update Email OTP recovery/export flow to use worker-owned handles or
      opaque state rather than exposing root-share bytes to core TypeScript.
  - [x] Email OTP ECDSA export uses the worker-owned authorization path and
        role-local boundary material.
  - [x] Email OTP Ed25519 export uses a worker-owned export command that returns
        only `{ publicKey, privateKey }` to the recovery UI flow.
...
- [x] Add parity fixtures for current WebAuthn PRF inputs, prepare output,
...
- [x] Add type fixtures proving EVM-family ECDSA provision plans reject
      top-level WebAuthn, Email OTP auth context, and root-share fields after
      `provisionSecretSource` wrapping.
- [x] Add type fixtures proving Ed25519 session mint authorization rejects raw
      credential and PRF fields after `localSecretSource` /
      `policySecretSource` wrapping.
...
- [x] Add export tests that use opaque state and reject missing public identity.
- [x] Run `npx tsc --noEmit -p sdk/tsconfig.build.json`.
- [x] Run targeted ECDSA HSS unit tests.
- [x] Run relevant `cargo test` commands for the Rust crate/WASM package touched
      by the coarse command.
  - [x] Run signer-core and ecdsa-hss cargo tests after Phase 6B restores
        `ecdsa-hss` protocol ownership.
  - [x] Run `wasm/hss_client_signer` cargo tests after the Phase 6B dependency
        direction correction.
  - [x] Rerun `wasm/hss_client_signer` cargo tests when the WASM binding changes
        for the coarse command.
- [x] Hand off before/after browser JS, WASM, and lazy-flow asset size recording
      to Refactor 51 Phase 6.

### High-Impact Successor Work

- [x] Track Phase 6 production cutover, generated schema ownership, raw
      bootstrap helper deletion, active raw-share deletion, data reset, and
      bundle-size measurement in Refactor 51 Phase 6 and Phase 9.
- [x] Track export/recovery ready-blob migration, fresh export authorization, and
      all-or-nothing export behavior in Refactor 51 Phase 7.
- [x] Track signing-session activation, exact refresh restore, same-method auth
      separation, and restored-session export rejection in Refactor 51 Phase 8.
...
5. Package-size regression.
   Measure WASM artifacts after any new signer-core command is exposed through
   the browser SDK.
...
- Do not keep both TypeScript helper pipelines and coarse signer-core commands
...
- At least one crypto-boundary slice has been internalized behind a coarse
  signer-core command and browser WASM binding with parity coverage.
- Existing browser registration, signing, recovery, HSS rebuild, and export
  flows remain green.
- Phase 7 and Phase 8 have no remaining Refactor 50 implementation work; only
  the high-impact successor tasks above remain in Refactor 51.
