...
Status: implemented in the working tree; GitHub history cleanup remains an
...
1. whether the workflow validates, deploys, or provides an internal
   implementation;
...
The final Actions sidebar must expose one deployment entrypoint per environment.
Service-specific deployment work remains visible as named jobs inside those
runs. Shared implementations are call-only and carry an `INTERNAL` prefix.
...
`deploy-router-ab` is especially misleading. It currently orchestrates Router
...
- internal reusable workflows look like supported entrypoints;
...
- the current call graph is difficult to audit from the Actions UI.
...
A workflow that cannot be run directly uses this grammar:

```text
INTERNAL / <Action> / <Platform service>
```

The initial internal workflow is:

```text
INTERNAL / release / cloudflare-stack
```

It accepts an environment from one of the two statically bound deployment
entrypoints. It exposes `workflow_call` only. It has no `workflow_dispatch`,
`push`, `pull_request`, or `workflow_run` trigger.
...
deploy / staging / cloudflare-router-ab
deploy / staging / cloudflare-gateway
deploy / staging / cloudflare-pages
...
The final `.github/workflows` directory contains eight workflows. The two
`INTERNAL` service workflows are call-only implementation details and do not
provide operator-facing dispatch buttons:

| File                                     | Actions sidebar name                     | Trigger                                                                  | Mutation authority                                       |
| ---------------------------------------- | ---------------------------------------- | ------------------------------------------------------------------------ | -------------------------------------------------------- |
| `validate-repository.yml`                | `Validate / repository`                  | Push, pull request, merge group, optional manual validation              | None                                                     |
...
| `internal-release-cloudflare-stack.yml`  | `INTERNAL / release / cloudflare-stack`  | `workflow_call` only                                                     | Authority explicitly passed by an environment entrypoint |
| `internal-deploy-cloudflare-stack.yml`   | `INTERNAL / deploy / cloudflare-stack`   | `workflow_call` only                                                     | Authority explicitly passed by an environment entrypoint |
| `internal-deploy-cloudflare-gateway.yml` | `INTERNAL / deploy / cloudflare-gateway` | `workflow_call` only                                                     | Called only by the stack workflow                        |
| `internal-deploy-cloudflare-pages.yml`   | `INTERNAL / deploy / cloudflare-pages`   | `workflow_call` only                                                     | Called only by the stack workflow                        |

The release builder lives in `internal-release-cloudflare-stack.yml`. The
environment-bound deploy orchestration lives in
`internal-deploy-cloudflare-stack.yml`, which calls the Gateway and Pages
implementation workflows. Shared command sequences may move into local scripts
or composite actions under `.github/actions`; those do not create additional
operator-facing deployment entrypoints.
...
| `build-release`                         | Builds accepted artifacts, creates a release set, then deploys      | `INTERNAL / release / cloudflare-stack`                                   |
...
| `deploy-router-ab`                      | Whole-stack orchestrator with an additional direct manual trigger   | Merge into `INTERNAL / release / cloudflare-stack`; remove direct trigger |
| `deploy-gateway`                        | Call-only Gateway deployment implementation                         | `deploy / <environment> / cloudflare-gateway` job                         |
| `deploy-pages`                          | Call-only Pages deployment implementation                           | `deploy / <environment> / cloudflare-pages` job                           |
| `validate-router-ab`                    | Router A/B pull-request and manual validation                       | `validate / cloudflare-router-ab` job in `Validate / repository`          |
...
push dev
    ...
    -> Deploy / staging / cloudflare-stack
    -> INTERNAL / release / cloudflare-stack
       -> build selected exact-SHA artifacts
       -> create immutable release set
       -> deploy / staging / cloudflare-router-ab
       -> deploy / staging / cloudflare-gateway
       -> deploy / staging / cloudflare-pages
       -> verify / staging / cloudflare-stack
...
push main
    ...
    -> INTERNAL / release / cloudflare-stack
       -> build selected exact-SHA artifacts
       -> create immutable release set
       -> deploy / production / cloudflare-router-ab
...
Deploy / <environment> / cloudflare-stack
    ...
    -> INTERNAL / release / cloudflare-stack
    -> deploy only the components named by the accepted release set
```

No deployment workflow calls an environment-generic manual workflow. No service
workflow can be dispatched directly.
...
10. Internal workflows expose `workflow_call` only.
11. Internal workflows receive explicit secrets. `secrets: inherit` is removed.
...
- [x] Reject `workflow_dispatch` on any `INTERNAL` workflow.
...
- [x] Assert the exact allowed workflow call graph.
- [x] Assert that only the two deployment entrypoints can call the internal
      release workflow.
...
## Phase 3: Create One Internal Release Implementation

- [x] Rename and reshape `build-release.yml` as
      `internal-release-cloudflare-stack.yml`.
- [x] Set its display name to `INTERNAL / release / cloudflare-stack`.
- [x] Remove `workflow_run` and `workflow_dispatch`; retain `workflow_call`
      only.
- [ ] Define narrow inputs for environment, source branch, source SHA,
      validation run ID, change-set identity, artifact mode, artifact run ID,
      and release-set ID.
- [ ] Model automatic build-and-deploy and manual accepted-release promotion as
      explicit modes with mutually exclusive required inputs.
- [x] Keep Router A/B deployment jobs in the call-only internal stack deployer.
- [x] Keep Gateway deployment jobs in the call-only internal Gateway workflow.
- [x] Keep Pages deployment jobs in the call-only internal Pages workflow.
...
- [ ] Move reusable step sequences into scripts or `.github/actions` when that
      reduces duplication without creating another visible workflow.
- [x] Delete `deploy-router-ab.yml`, `deploy-gateway.yml`, and
      `deploy-pages.yml`.
...
- [x] Add environment-qualified `run-name` values for automatic and manual
      modes.
- [x] Remove inherited secrets from reusable-workflow calls; called jobs resolve
      only their declared target environment secrets.
...
- [ ] Run production final smoke against `seams.sh` and its backend readiness
      endpoints.
- [ ] Verify the Actions sidebar matches the target workflow surface.

## Rollback

Application rollback uses the same environment-specific deployment entrypoint
with a previously accepted release set. It does not restore old workflow names
or direct service dispatch paths.
...
- [ ] The Actions sidebar contains the eight current workflows plus
...
- [ ] Internal workflows are call-only.
...
- [ ] Parsed workflow policy tests reject ambiguous names and unauthorized
      triggers.
- [ ] Deployment documentation contains one authoritative call graph and one
