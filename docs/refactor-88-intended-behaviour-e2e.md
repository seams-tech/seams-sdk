...
| `tests/unit/demoThresholdHooks.actions.unit.test.ts` | keep | Focused demo hook/component coverage. It fakes the signing backend to check visible threshold-owner rendering, Tempo fee-token post-finalization refreshes, and Tempo/Arc demo action request construction. The intended contracts own lifecycle correctness; this test still owns demo-specific UI/action wiring. |
...
| `tests/unit/passkeyAuthMenu.accountAvailability.unit.test.ts` | keep | Passkey auth menu account-availability coverage. It verifies local saved credentials do not imply chain registration and badge state stays neutral until server existence is known. |
| `tests/unit/passkeyAuthMenu.fouc.unit.test.ts` | keep | Passkey auth menu UI/state coverage. It verifies style bootstrap, auth-method labels, Email OTP handoff prompts, resend UI, recovery options, and dropdown rendering. |
...
| `tests/wallet-iframe/csp.strict.violation-free.test.ts` | keep | Strict wallet-service CSP coverage. It verifies the wallet-service page has no inline style tags or style attributes under strict dev CSP, outside the lifecycle contract matrix. |
...
| `tests/wallet-iframe/passkeyAuthMenu.qrButton.overlay.test.ts` | keep | QR-button overlay regression coverage. It ensures Scan and Link Device does not surface the wallet iframe overlay, a UI boundary outside intended lifecycle success. |
...
| `tests/unit/emailRecoveryVerifiedRequest.source.script.ts` | keep | Source fixture for email recovery verified-request tests. It has a surviving unit-test importer and deletes with that importer. |
| `tests/unit/emailRecoveryVerifiedRequest.source.script.unit.test.ts` | keep | Email recovery verified-request source test. It validates source-script parsing and evidence for recovery boundaries. |
...
| `tests/unit/passkeyAuthMenu.ssr.unit.test.ts` | keep | Passkey auth menu SSR coverage. It validates server-render safety for the UI surface. |
...
| `tests/unit/profileContinuity.source.script.ts` | keep | Source fixture for profile-continuity tests. It has a surviving unit-test importer and deletes with that importer. |
| `tests/unit/profileContinuity.source.script.unit.test.ts` | keep | Profile-continuity source-script coverage. It validates source evidence for profile continuity boundaries. |
...
| `tests/unit/recoveryDomain.source.script.ts` | keep | Source fixture for recovery-domain tests. It has a surviving unit-test importer and deletes with that importer. |
| `tests/unit/recoveryDomain.source.script.unit.test.ts` | keep | Recovery-domain source-script coverage. It validates recovery source evidence and domain boundaries. |
