...
1. [ ] Add a shared `requestEmailOtpResend` helper or reuse the existing challenge helper with explicit `resend: true` metadata.
2. [ ] Require resend callers to pass the same operation context as the original challenge.
3. [ ] Return `challengeId`, `expiresAtMs`, optional `emailHint`, and optional `retryAfterMs`.
4. [ ] Ensure all Email OTP UI flows can update their active challenge ID after resend.
5. [ ] Preserve the app-session auth lane for resend challenge requests.

### Phase 2: UI Integration

1. [ ] Add `Resend code` to wallet unlock Email OTP UI.
2. [ ] Add `Resend code` to transaction-signing Email OTP UI.
3. [ ] Add `Resend code` to private-key export Email OTP UI.
4. [ ] Add `Resend code` to registration Email OTP UI.
5. [ ] Add 10-second client debounce.
6. [ ] Show countdown text while debounce is active.
7. [ ] Keep OTP input contents when resend is clicked.
8. [ ] Handle `rate_limited` with clear copy and retry hint.

### Phase 3: Server Challenge Semantics

1. [ ] Verify the server permits multiple active OTP challenges for the same context.
2. [ ] Ensure each challenge has independent TTL and single-use state.
3. [ ] Ensure verification can succeed for any active matching challenge/code.
4. [ ] Prune expired challenges during challenge creation and verification.
5. [ ] Enforce max active challenge count per context.
6. [ ] Add structured `retryAfterMs` for rate-limited responses where practical.
7. [ ] Ensure resend does not bypass abuse counters.

### Phase 4: Tests

1. [ ] Unit test resend button debounce.
2. [ ] Unit test resend preserves OTP input state.
3. [ ] Unit test resend updates current `challengeId`.
4. [ ] Unit test rate-limit errors show the right UI copy.
5. [ ] Server test two issued codes both verify before TTL.
6. [ ] Server test a used code cannot verify again.
7. [ ] Server test expired code fails while newer unexpired code succeeds.
8. [ ] Server test resend keeps wallet/user/operation/app-session binding.
9. [ ] E2E test Email OTP unlock with resend.
10. [ ] E2E test Email OTP export with resend.
11. [ ] E2E test transaction-signing OTP with resend in `per_operation` mode.

## Master TODO

1. [ ] Freeze resend semantics: multiple active codes are valid until TTL and single-use consumption.
2. [ ] Add client-side 10-second resend debounce.
3. [ ] Add `Resend code` UI to all Email OTP screens.
4. [ ] Ensure resend reuses the exact original wallet/user/channel/operation/session context.
5. [ ] Ensure server rate limiting remains authoritative and returns useful errors.
6. [ ] Ensure all active unexpired codes can verify.
7. [ ] Add cleanup for expired active challenges.
8. [ ] Add unit tests for UI debounce and challenge replacement.
9. [ ] Add server tests for multi-code validity, single-use, TTL, and context binding.
10. [ ] Add E2E coverage for unlock, export, and per-operation signing resend flows.
