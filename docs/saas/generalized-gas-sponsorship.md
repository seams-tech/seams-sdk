...
- the shared sponsorship pricing path now supports:
  ...
  - optional on-chain Outlayer pricing via `SPONSORED_EXECUTION_REAL_PRICING_JSON`
...
- NEAR `near_delegate` policies can now enforce spend caps through the same shared budgeting path using:
  ...
  - shared Outlayer NEAR/USD pricing support
...
- optional future work only:
  - additional on-chain pricing sources beyond the current Outlayer adapter
...
- [x] Use the mainnet `price-oracle.near` multi-source median with freshness and EMA-deviation checks as the real NEAR pricing source
...
- [ ] Optional future hardening:
  ...
  - an owned sampled moving average if the external oracle trust boundary stops being sufficient
