...
- [x] Expose `registration.addEmailOtp` through the wallet API and iframe
      surfaces.
- [ ] Prove same-authority sibling revocation in both directions through the
      composed server and IndexedDB path, including the final-method guard.
- [ ] Complete and accept Passkey-to-Email-OTP in the browser, including the
      security note, source selection retention, unlock, signing, export,
      step-up, and sibling revocation.
- [ ] Complete and accept Email-OTP-to-Passkey through the same internal
      operation and operating path.
