export function friendlyWebAuthnMessage(err: any): string {
  ...
  const notAllowed =
    ...
    /timed out or was not allowed/i.test(msg);
  ...
  if (name === 'TimeoutError' || /timed out/i.test(msg))
    return 'Touch ID timed out. Please try again.';
  if (name === 'SecurityError' || /SecurityError/i.test(msg))
    return 'Security error. Make sure you are on a secure site (HTTPS).';
  if (name === 'InvalidStateError' || /InvalidStateError/i.test(msg))
    return 'No matching passkey found for this account.';
