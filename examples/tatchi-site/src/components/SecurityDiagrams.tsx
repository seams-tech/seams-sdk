...
const SecurityCustodySvg = loadInlineSvg(
  () => import('../assets/diagrams/security-custody.svg?raw'),
);
const SecurityDefenseSvg = loadInlineSvg(
  () => import('../assets/diagrams/security-defense.svg?raw'),
);
const SecurityScaleSvg = loadInlineSvg(() => import('../assets/diagrams/security-scale.svg?raw'));
const SecurityFrictionSvg = loadInlineSvg(
  () => import('../assets/diagrams/security-friction.svg?raw'),
);
...
function buildDiagramStyle(props: SecurityDiagramProps): React.CSSProperties {
  ...
  if (props.canvasColor) {
    ...
    vars['--security-diagram-canvas-top'] =
      `color-mix(in srgb, ${props.canvasColor} 86%, var(--site-canvas) 14%)`;
    vars['--security-diagram-canvas-bottom'] =
      `color-mix(in srgb, ${props.canvasColor} 68%, var(--site-canvas) 32%)`;
  }
  if (props.lineColor) {
    vars['--security-diagram-line'] = props.lineColor;
    vars['--security-diagram-line-strong'] =
      `color-mix(in srgb, ${props.lineColor} 78%, var(--site-text-primary) 22%)`;
  }
...
function renderSecurityDiagram(
  ...
  const style = buildDiagramStyle(props);

  return (
    <React.Suspense
      fallback={
        <img className={props.className} src={fallbackSrc} alt={alt} loading="lazy" style={style} />
      }
    >
      <Component className={props.className} style={style} alt={alt} />
    </React.Suspense>
  );
}
