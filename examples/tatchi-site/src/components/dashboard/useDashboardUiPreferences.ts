...
function sanitizeSelectedContext(input: Partial<TopbarContextState> | undefined): TopbarContextState {
  return {
    organization: input?.organization && isTopbarOption('organization', input.organization)
      ? input.organization
      : DEFAULT_TOPBAR_CONTEXT_STATE.organization,
    project: input?.project && isTopbarOption('project', input.project)
      ? input.project
      : DEFAULT_TOPBAR_CONTEXT_STATE.project,
    environment: input?.environment && isTopbarOption('environment', input.environment)
      ? input.environment
      : DEFAULT_TOPBAR_CONTEXT_STATE.environment,
    accountSettings: input?.accountSettings && isTopbarOption('accountSettings', input.accountSettings)
      ? input.accountSettings
      : DEFAULT_TOPBAR_CONTEXT_STATE.accountSettings,
  }
}

function sanitizeExpandedGroups(input: Partial<ExpandedSidebarGroupsState> | undefined): ExpandedSidebarGroupsState {
  return {
    walletInfrastructure: input?.walletInfrastructure ?? DEFAULT_EXPANDED_SIDEBAR_GROUPS.walletInfrastructure,
    securityPolicy: input?.securityPolicy ?? DEFAULT_EXPANDED_SIDEBAR_GROUPS.securityPolicy,
    integrationsAutomation: input?.integrationsAutomation ?? DEFAULT_EXPANDED_SIDEBAR_GROUPS.integrationsAutomation,
    environmentSettings: input?.environmentSettings ?? DEFAULT_EXPANDED_SIDEBAR_GROUPS.environmentSettings,
  }
}
...
function readStoredState(): Partial<PersistedUiState> {
  ...
  try {
    ...
    return {
      isSidebarExpanded: typeof parsed.isSidebarExpanded === 'boolean' ? parsed.isSidebarExpanded : undefined,
...
function parseExpandedGroupsParam(raw: string | null): Partial<ExpandedSidebarGroupsState> | undefined {
  if (!raw) return undefined
  const active = new Set(raw.split(',').map((value) => value.trim()).filter(Boolean))
...
function readUrlState(): Partial<PersistedUiState> {
  ...
  const sidebarExpanded = sidebarExpandedRaw === '1'
    ? true
    : sidebarExpandedRaw === '0'
      ? false
      : undefined

  return {
    isSidebarExpanded: sidebarExpanded,
    expandedGroups: sanitizeExpandedGroups(parseExpandedGroupsParam(params.get(QUERY_KEYS.expandedGroups))),
...
function readInitialState(): PersistedUiState {
  ...
  return {
    isSidebarExpanded: fromUrl.isSidebarExpanded ?? stored.isSidebarExpanded ?? defaults.isSidebarExpanded,
...
export function useDashboardUiPreferences(pathname: string): UseDashboardUiPreferencesResult {
  ...
  const [isSidebarExpanded, setIsSidebarExpanded] = React.useState<boolean>(initialState.isSidebarExpanded)
  const [expandedGroups, setExpandedGroups] = React.useState<ExpandedSidebarGroupsState>(initialState.expandedGroups)
  const [selectedContext, setSelectedContext] = React.useState<TopbarContextState>(initialState.selectedContext)
