...
const config = {
  ...
  iframeWallet: { walletOrigin: 'https://wallet.web3authn.org' },
  relayer: { url: 'https://relay.seams.xyz' },
};
...
`PasskeyAuthMenu` is a ready‑made registration/unlock/account-sync menu that wires into the passkey flows exposed by `useSeams`.
...
import { PasskeyAuthMenu } from '@seams/sdk/react/passkey-auth-menu';

export function PasskeySection() {
  const { seams, accountInputState, registerPasskey } = useSeams();
  ...
  const onLogin = () =>
    seams.auth.unlock(targetAccountId, {
      onEvent: (event) => {
...
```tsx
import { useSeams, DeviceLinkingPhase, DeviceLinkingStatus } from '@seams/sdk/react';
...
```tsx
import { useSeams, ActionType, TxExecutionStatus } from '@seams/sdk/react';
