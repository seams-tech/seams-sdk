...
Then add the following Seams plugins to your `vite.config.ts` file:
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [react(), seamsAppServer({ walletOrigin }), seamsBuildHeaders({ walletOrigin })],
  };
});
```
  ...
  return (
    <main>
      ...
      <button
        onClick={() => {
          const id = Date.now();
          seams.registration.registerPasskey(`seams-test-${id}.${configs.relayerAccount}`, {
            onEvent: (event) => console.log('registration event: ', event),
          });
        }}
      >
        Register Seams Account
      </button>
    </main>
  );
}
