...
export type SeamsLogoProps = React.SVGProps<SVGSVGElement> & {
  size?: number | string;
  strokeWidth?: number;
};
...
const SeamsLogo: React.FC<SeamsLogoProps> = ({
  ...
  return (
    <svg
      ...
      className={['lucide', 'lucide-orbit-icon', 'lucide-orbit', 'seams-logo-icon', className]
        .filter(Boolean)
        .join(' ')}
      aria-hidden="true"
      {...rest}
    >
      ...
      <g transform={`translate(12 12) scale(${scale}) translate(-12 -12)`}>
        <path
          className="seams-logo--fingerprint"
