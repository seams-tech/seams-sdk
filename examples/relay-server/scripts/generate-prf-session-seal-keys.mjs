...
function usage() {
  console.log(`Generate Shamir 3-pass key material for PRF session seal routes.

Usage:
  pnpm -C examples/relay-server prf-seal:keygen [--key-version <value>] [--prime-bits <bits>] [--json]
...
function printEnvBlocks(material) {
  ...
  console.log('PRF_SESSION_SEAL_ENABLED=1');
  console.log(`PRF_SESSION_SEAL_KEY_VERSION=${keyVersion}`);
  console.log(`SHAMIR_P_B64U=${shamirPrimeB64u}`);
  console.log(`SHAMIR_E_S_B64U=${serverEncryptExponentB64u}`);
  console.log(`SHAMIR_D_S_B64U=${serverDecryptExponentB64u}`);
  console.log('');
...
main().catch((error) => {
  console.error(`[prf-seal:keygen] ${error instanceof Error ? error.message : String(error)}`);
  process.exit(1);
});
