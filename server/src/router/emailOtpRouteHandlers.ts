...
export async function handleEmailOtpRecoveryKeyBackupAcknowledgeRoute(input: {
  body: unknown;
  claims: Record<string, unknown>;
  userId: string;
  appSessionVersion: string;
  ...
  const body = bodyValidation.body;
  for (const field of ['recoveryKey', 'recoveryKeys', 'recoveryKeyB64u', 'recoveryKek', 'encS', 'encSB64u']) {
    if (Object.prototype.hasOwnProperty.call(body, field)) {
      return {
        ...
        body: {
          ok: false,
          code: 'invalid_body',
          message: 'Recovery-code backup acknowledgements must not include recovery-key material',
        },
      };
    }
  }
  ...
  const enrollmentIdValidation = validateEmailOtpRequiredString(body, 'enrollmentId');
  if (!enrollmentIdValidation.ok) {
    return { status: enrollmentIdValidation.status, body: enrollmentIdValidation.body };
  }
  const enrollmentSealKeyVersionValidation = validateEmailOtpRequiredString(
    body,
    'enrollmentSealKeyVersion',
  );
  if (!enrollmentSealKeyVersionValidation.ok) {
    return {
      status: enrollmentSealKeyVersionValidation.status,
      body: enrollmentSealKeyVersionValidation.body,
    };
  }
  ...
  const result = await input.service.acknowledgeEmailOtpRecoveryCodeBackup({
    userId: providerUser.providerUserId,
    walletId: walletValidation.walletId,
    orgId: readEmailOtpOrgIdFromClaims(input.claims),
    enrollmentId: enrollmentIdValidation.value,
    enrollmentSealKeyVersion: enrollmentSealKeyVersionValidation.value,
    clientIp: input.clientIp,
  });
  return { status: emailOtpResultStatus(result), body: result };
}
