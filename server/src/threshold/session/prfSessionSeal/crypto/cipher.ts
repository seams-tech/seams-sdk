...
type HandlerInput = Omit<PrfSessionSealCipherOperationInput, 'operation'>;
...
function normalizeResult(result: HandlerResult): HandlerResult {
  ...
  return {
    ...
    message: String(result.message || 'PRF seal cipher failed').trim() || 'PRF seal cipher failed',
  };
}

export function createPrfSessionSealCipherAdapter(
...
export function createPassthroughPrfSessionSealCipherAdapter(): PrfSessionSealCipherAdapter {
...
export interface PrfSessionSealShamir3PassRuntimeInput {
...
export interface CreatePrfSessionSealShamir3PassCipherAdapterOptions {
  currentKeyVersion: string;
  keys: PrfSessionSealShamir3PassKeyMaterial[];
  runtime?: PrfSessionSealShamir3PassRuntime;
...
function runShamirModExp(input: PrfSessionSealShamir3PassRuntimeInput): string {
...
export function createPrfSessionSealShamir3PassBigIntRuntime(): PrfSessionSealShamir3PassRuntime {
  return {
...
function normalizeKeyMaterial(
  input: PrfSessionSealShamir3PassKeyMaterial,
  index: number,
...
function cipherFailure(code: string, message: string): PrfSessionSealCipherOperationResult {
  return { ok: false, code, message };
}
...
export function createPrfSessionSealShamir3PassCipherAdapter(
  options: CreatePrfSessionSealShamir3PassCipherAdapterOptions,
): PrfSessionSealCipherAdapter {
  ...
  const keyByVersion = new Map<string, PrfSessionSealShamir3PassKeyMaterial>();
  ...
  const runtime = options.runtime || createPrfSessionSealShamir3PassBigIntRuntime();
