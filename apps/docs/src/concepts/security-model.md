...
```ts
import { seamsBuildHeaders } from '@seams/sdk/plugins/vite';

plugins: [seamsBuildHeaders({ walletOrigin: process.env.VITE_WALLET_ORIGIN })];
```
...
```ts
import { ActionPhase } from '@seams/sdk/react';

await seams.near.executeAction({
...
For more details, see the [Architecture](/concepts/architecture) guide.
...
See [SecureConfirm Sessions](/concepts/secureconfirm-sessions) for the detailed handshake flow.
...
- Learn about [passkey scope strategies](/concepts/passkey-scope)
- Understand [SecureConfirm-backed challenges](/concepts/secureconfirm-webauthn)
- Review the [architecture and iframe isolation model](/concepts/architecture)
