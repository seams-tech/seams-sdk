...
Seams wallets use the WebAuthn **PRF extension** on every flow: `PRF.first` gates sessions (with Shamir 3-pass as primary) and `PRF.second` is reserved for registration, device linking, or explicit recovery. Whichever rpId strategy you choose must remain stable so these PRF outputs stay discoverable.
...
```json
{
  "origins": ["https://wallet.seams.com", "https://web3authn.org", "https://example.your-app.com"]
}
```
...
- Learn about [SecureConfirm-backed challenges](/concepts/secureconfirm-webauthn)
- Understand the [architecture and iframe isolation model](/concepts/architecture)
- Review the [security model](/concepts/security-model)
