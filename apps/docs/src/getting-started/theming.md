---
title: Theming
description: Customize Seams colors and shapes across React components and protected wallet-iframe surfaces.
---
...
<Theme theme="light" tokens={{ light: { colors: myColors, shape: SHAPE_PRESETS.rounded } }}>
  <SeamsAuthMenu onOutcome={(outcome) => console.log('auth menu outcome', outcome)} />
</Theme>;
```
...
<Theme
  theme={preset.mode}
  tokens={{ [preset.mode]: { colors: preset.colors, shape: preset.shape } }}
>
  {children}
</Theme>;
